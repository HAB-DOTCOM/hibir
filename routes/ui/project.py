...
from system.security_functions import (
...
def check_project_access(fn):
    ...
    def decorated_view(*args, **kwargs):
        ...
        current_project = db.check_user_project_access(str(project_id), session["id"])
...
"""def check_admin_access(fn):
    ...
    def decorated_view(*args, **kwargs):
        user_id = session["id"]
        d = db.select_user_by_id(user_id)
        if d and any(user["admin"] == 1 for user in d):
            return fn(*args, **kwargs)
...
def check_issue_template_access(fn):
    ...
    def decorated_view(*args, **kwargs):
        ...
        current_template = db.check_user_issue_template_access(
            template_id, current_user["id"], current_user["email"]
        )
...
def check_chat_access(fn):
    ...
    def decorated_view(*args, **kwargs):
        ...
        current_chat = db.select_project_chat(current_project["id"], str(chat_id))
        if not current_chat:
            return redirect("/projects/")
...
def check_project_archived(fn):
    ...
    def decorated_view(*args, **kwargs):
        ...
        if current_project["status"] == 0 or (
            current_project["end_date"] < time.time()
            and current_project["auto_archive"] == 1
        ):
...
def check_project_creds(fn):
    ...
    def decorated_view(*args, **kwargs):
        ...
        if not current_creds:
            return redirect("/project/{}/credentials/".format(current_project["id"]))
...
def check_project_network(fn):
    ...
    def decorated_view(*args, **kwargs):
        ...
        current_network = db.select_project_networks_by_id(
            current_project["id"], str(network_id)
        )
...
def getFieldFile(file_id):
    return send_from_directory(
        ...
        str(file_id),
        as_attachment=True,
        attachment_filename=db.select_files(str(file_id))[0]["filename"],
    )


@routes.route("/about", methods=["GET"])
def about_page():
    return render_template("about.html", tab_name="About")


@routes.route("/forbidden/", methods=["GET"])
def forbidden_access():
    return render_template("405.html", tab_name="Forbidden Access")
...
@check_password_reset
@check_admin_access
def admin_page(*args, **kargs):

    return render_template("admin.html", tab_name="Admin Page")
    # return redirect("/coming-soon")


@routes.route("/coming-soon", methods=["GET"])
@check_session
@check_password_reset
@check_admin_access
def coming_soon(*args, **kargs):
    return render_template("coming_soon.html", tab_name="Under Construction")


@routes.route("/admin/", methods=["POST"])
@check_session
@check_admin_access
def admin_page_interface(*args, **kargs):
    form = ChangePassword()
    form.validate()
    errors = []
    if form.errors:
    ...
    if not errors:
        # Resetting Password
        db.reset_password(form.password1.data, form.user_id.data)
        success_type = "reset_password"
...
def hosts(project_id, current_project, current_user):
    return render_template(
        "project/hosts/list.html", current_project=current_project, tab_name="Hosts"
    )
...
def new_host(project_id, current_project, current_user):
    return render_template(
        "project/hosts/new.html", current_project=current_project, tab_name="Add host"
    )
...
@routes.route("/project/<uuid:project_id>/host/<uuid:host_id>/", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
@routes.route("/project/<uuid:project_id>/host/<uuid:host_id>/", methods=["POST"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def host_page_form(project_id, host_id, current_project, current_user):
    ...
    if "update_description" in request.form:
        ...
        if not errors:
            ...
            db.update_host_comment_threats(
                current_host["id"], form.comment.data, form.threats.data, os
            )
            current_host = db.select_project_host(current_project["id"], str(host_id))[
                0
            ]
    ...
    if "add_port" in request.form:
        ...
        if not errors:
            ...
            service = (
                form.service_text.data if form.service_text.data else form.service.data
            )
            found = {}
            for port in ports:
            ...
            if not found:
                ...
                db.update_port_proto_description(
                    found["id"], service, form.description.data
                )
    ...
    if "add_hostname" in request.form:
        ...
        if not errors:
            hostnames = db.find_ip_hostname(current_host["id"], form.hostname.data)
            if hostnames:
    ...
    if "delete_hostname" in request.form:
        ...
        if not errors:
            found = db.check_host_hostname_id(current_host["id"], form.hostname_id.data)
            if not found:
...
@routes.route("/project/<uuid:project_id>/hosts/multiple_delete", methods=["POST"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def new_issue(project_id, current_project, current_user):
    return render_template(
        "project/issues/new.html", current_project=current_project, tab_name="New issue"
    )
...
def new_issue_form(project_id, current_project, current_user):
    ...
    if not errors:
        ...
        file_move_list = []
        if (
            len(form.additional_field_name.data)
            ...
            for field_name in form.additional_field_name.data:
                ...
                field_counter += 1
        if len(form.additional_field_file.data) == len(
            form.additional_field_filename.data
        ):
            file_counter = 0
            for file_field_name in form.additional_field_filename.data:
                ...
                if file_size > int(config["files"]["poc_max_size"]):
                    ...
                    file_move_list.append(
                        [tmp_file_path, file_path, field_id, filename]
                    )
...
@routes.route("/project/<uuid:project_id>/issue/<uuid:issue_id>/", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
@check_project_issue
@send_log_data
def issues_info(project_id, issue_id, current_project, current_user, current_issue):
    return render_template(
        "project/issues/edit.html",
        current_project=current_project,
...
@routes.route(
    "/project/<uuid:project_id>/issue/<uuid:issue_id>/template/<uuid:template_id>/",
    methods=["GET"],
)
@requires_authorization
@check_session
...
def edit_issue_with_template(
    ...
    return render_template(
        "project/issues/with_template.html",
        current_project=current_project,
        current_issue=current_issue,
        tab_name=current_issue["name"] if current_issue["name"] else "Issue",
        current_user=current_user,
...
@check_project_archived
@send_log_data
def edit_issue_with_template_form(
    project_id,
    issue_id,
    template_id,
    current_project,
    ...
    if not (
        len(form.variable_value.data)
        == len(form.variable_type.data)
        == len(form.variable_name.data)
    ):
    ...
    if not errors:
        ...
        for variable_name in form.variable_name.data:
            ...
            if variable_type in ["text", "number", "float", "boolean"]:
                ...
                try:
                    ...
                    add_variables_dict[variable_name]["value"] = (
                        type_func(variable_value)
                        if variable_type == "text" or variable_value
                        else type_func(0)
                    )
        ...
        for field_name in issue_fields:
            if issue_fields[field_name]["type"] == "text":
                issue_fields[field_name]["value"] = replace_tpl_text(
                    issue_fields[field_name]["value"]
                )
        ...
        return redirect(
            "/project/{}/issue/{}/".format(current_project["id"], current_issue["id"])
        )
...
@routes.route(
    "/project/<uuid:project_id>/issue/<uuid:issue_id>/hosts_ports_list", methods=["GET"]
)
@requires_authorization
@check_session
@check_password_reset
...
@check_project_issue
@send_log_data
def filter_host_port_form(
    project_id, issue_id, current_project, current_user, current_issue
):
    ...
    for port in ports_list:
        port["checked"] = False
        if (
            port["port_id"] in issue_service_dict
            and "0" in issue_service_dict[port["port_id"]]
        ):
            port["checked"] = True
    ...
    for port in hostnames_list:
        ...
        if (
            port["port_id"] in issue_service_dict
            and port["hostname_id"] in issue_service_dict[port["port_id"]]
        ):
            port["checked"] = True
...
@routes.route("/project/<uuid:project_id>/issues/hosts_ports_list", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
@routes.route("/project/<uuid:project_id>/issues/hosts_hostnames_list", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
@routes.route("/project/<uuid:project_id>/issues/networks_list", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def networks_list(project_id, current_project, current_user):
    hosts_hostnames = [
        {"id": x["id"], "network": "{}/{}".format(x["ip"], x["mask"])}
        for x in db.select_project_networks(current_project["id"])
    ]
    return json.dumps(hosts_hostnames)


@routes.route("/project/<uuid:project_id>/issues/hosts_list", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def hosts_list(project_id, current_project, current_user):
    hosts = [
        {"id": x["id"], "ip": x["ip"]}
        for x in db.select_project_hosts(current_project["id"])
    ]
    return json.dumps(hosts)


@routes.route("/project/<uuid:project_id>/issue/<uuid:issue_id>/", methods=["POST"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
@routes.route(
    "/project/<uuid:project_id>/issue/<uuid:issue_id>/edit_text_fields",
    methods=["POST"],
)
@requires_authorization
@check_session
...
def issues_edit_fields(
    ...
    if not errors:
        ...
        for field_name in old_field_dict:
            ...
            if field_type == "file":
                add_fields_dict[field_name] = {"type": field_type, "value": field_value}
    ...
    return redirect(
        "/project/{}/issue/{}/#/fields".format(
            current_project["id"], current_issue["id"]
        )
    )


@routes.route(
    "/project/<uuid:project_id>/issue/<uuid:issue_id>/edit_file_fields",
    methods=["POST"],
)
@requires_authorization
@check_session
...
def issues_edit_file_fields(
    ...
    if not errors:
        ...
        # add new fields
        if len(form.additional_field_name.data) == len(form.additional_field_file.data):
            for field_name, field_file in zip(
                form.additional_field_name.data, form.additional_field_file.data
            ):
...
@routes.route(
    "/project/<uuid:project_id>/issue/<uuid:issue_id>/delete_issue", methods=["POST"]
)
@requires_authorization
@check_session
@check_password_reset
...
@routes.route(
    "/project/<uuid:project_id>/issue/<uuid:issue_id>/new_poc", methods=["POST"]
)
@requires_authorization
@check_session
@check_password_reset
...
@check_project_issue
@send_log_data
def new_poc_form(project_id, issue_id, current_project, current_user, current_issue):
    form = NewPOC()
    ...
    if not errors:
        ...
        if not ((port_id == "0" or port_id == 0) and (hostname_id == "0" or hostname_id == 0)):
            # check if port-host in issue
            if not db.check_hostname_port_in_issue(
                hostname_id, port_id, current_issue["id"]
            ):
...
@routes.route(
    "/project/<uuid:project_id>/issue/<uuid:issue_id>/delete_poc", methods=["POST"]
)
@requires_authorization
@check_session
@check_password_reset
...
@routes.route(
    "/project/<uuid:project_id>/issue/<uuid:issue_id>/edit_field", methods=["POST"]
)
@requires_authorization
@check_session
@check_password_reset
...
@routes.route(
    "/project/<uuid:project_id>/issue/<uuid:issue_id>/set_priority", methods=["POST"]
)
@requires_authorization
@check_session
@check_password_reset
...
@routes.route("/project/<uuid:project_id>/networks/", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def networks(project_id, current_project, current_user):
    return render_template(
        "project/networks/list.html",
        current_project=current_project,
        tab_name="Networks",
    )


@routes.route("/project/<uuid:project_id>/networks/graph", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def networks_graph(project_id, current_project, current_user):
    return render_template(
        "project/networks/graph.html",
        current_project=current_project,
        tab_name="Networks graph",
    )


@routes.route("/project/<uuid:project_id>/networks/graph.json", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def networks_graph_json(project_id, current_project, current_user):
    ...
    for current_network in networks:
        ...
        current_network["ip_obj"] = ipaddress.ip_network(
            "{}/{}".format(current_network["ip"], current_network["mask"]), False
        )

    j.append({"group": "nodes", "data": {"id": "network_0", "name": "0.0.0.0/0"}})
...
@routes.route("/project/<uuid:project_id>/networks/new_network", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def new_network(project_id, current_project, current_user):
    return render_template(
        "project/networks/new.html",
        current_project=current_project,
        tab_name="New network",
    )


@routes.route("/project/<uuid:project_id>/networks/new_network", methods=["POST"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def new_network_form(project_id, current_project, current_user):
    ...
    if not errors:
        exists_network = db.select_project_network_by_ip(
            current_project["id"], form.ip.data, form.mask.data
        )
        if exists_network:
...
@routes.route("/project/<uuid:project_id>/networks/<uuid:network_id>/", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
@send_log_data
def project_network_info(
    project_id, current_project, current_user, network_id, current_network
):
    return render_template(
        "project/networks/edit.html",
        current_project=current_project,
...
@routes.route(
    "/project/<uuid:project_id>/networks/<uuid:network_id>/edit", methods=["POST"]
)
@requires_authorization
@check_session
@check_password_reset
...
def project_network_edit(
    ...
    # check if network exists
    if not errors:
        exists_networks = db.select_project_network_by_ip(
            current_project["id"], form.ip.data, form.mask.data
        )
    ...
    if not errors:
        ...
        return redirect(
            "/project/{}/networks/{}/".format(
                current_project["id"], current_network["id"]
            )
        )
...
@routes.route("/project/<uuid:project_id>/networks/add_path", methods=["POST"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def new_path_form(project_id, current_project, current_user):
    ...
    if not errors:
        if form.type_out.data == "host":
            ...
            if (
                not current_host
                or current_host[0]["project_id"] != current_project["id"]
            ):
            ...
            if (
                not current_network
                or current_network[0]["project_id"] != current_project["id"]
            ):
    ...
    added = 0
    if not errors:
        if (network_in != "" and network_in == network_out) or (
            host_in != "" and host_in == host_out
        ):
...
@routes.route("/project/<uuid:project_id>/networks/delete_path", methods=["POST"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
    ...
    if form.errors:
        for field in form.errors:
            for error in form.errors[field]:
                errors.append(error)
    if not errors:
        db.delete_path(path_id=form.path_id.data, project_id=current_project["id"])

    return redirect("/project/{}/networks/#/paths".format(current_project["id"]))


@routes.route("/project/<uuid:project_id>/credentials/", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def project_credentials(project_id, current_project, current_user):
    return render_template(
        "project/creds/list.html", current_project=current_project, tab_name="Users"
    )


@routes.route("/project/<uuid:project_id>/credentials/new_creds", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def project_add_credentials(project_id, current_project, current_user):
    return render_template(
        "project/creds/new.html", current_project=current_project, tab_name="New User"
    )


@routes.route("/project/<uuid:project_id>/credentials/new_creds", methods=["POST"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def project_add_credentials_form(project_id, current_project, current_user):
    ...
    if not errors:
        if (
            form.check_pwd.data != ""
            and form.password_hash.data != ""
            and form.hash_type.data != ""
        ):
            ...
            if form.hash_type.data == "md5_hex":
                ...
                try:
                    ...
                    hash_function = lambda clr_str: md5_crypt_str(clr_str, salt)
                except Exception as e:
                    pass
            elif form.hash_type.data == "rabbitmq_md5":
                try:
                    salt = base64.b64decode(form.password_hash.data)[:4]
                    hash_function = lambda clr_str: rabbitmq_md5_str(clr_str, salt)
                except Exception as e:
                    pass
            elif form.hash_type.data == "des_crypt_unix":
                try:
                    salt = form.password_hash.data[:2]
                    hash_function = lambda clr_str: des_crypt_str(clr_str, salt)
                except Exception as e:
                    pass
            elif form.hash_type.data == "sha512_crypt_unix":
                try:
                    salt = form.password_hash.data.split("$")[2]
                    hash_function = lambda clr_str: sha512_crypt_str(clr_str, salt)
                except Exception as e:
                    pass
            elif form.hash_type.data == "sha256_crypt_unix":
                try:
                    salt = form.password_hash.data.split("$")[2]
                    hash_function = lambda clr_str: sha256_crypt_str(clr_str, salt)
                except Exception as e:
                    pass
            ...
            if dict_file_path != "" and hash_function != "":
                ...
                for cleartext_pwd in dict_list:
                    if hash_function(cleartext_pwd) == form.password_hash.data.strip():
                        form.cleartext_password.data = cleartext_pwd

    if not errors:
        creds_id = db.insert_new_cred(
            form.login.data,
...
@routes.route("/project/<uuid:project_id>/credentials/import_creds", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def project_add_credentials_multiple(project_id, current_project, current_user):
    return render_template(
        "project/creds/multiple.html",
        current_project=current_project,
        tab_name="Many Users",
    )


@routes.route("/project/<uuid:project_id>/credentials/import_creds", methods=["POST"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def project_add_credentials_multiple_form(project_id, current_project, current_user):
    ...
    if "file" in request.files and request.files.get("file").filename:
        ...
        tmp_path = path.join(config["main"]["tmp_path"], template_tmp_name)
        file.save(tmp_path)
        file.close()
    ...
    if form.do_not_check_columns.data != "1":
        ...
        if error:
            errors.append(
                "Some rows are incorrect! len(columns) less than index of searching column"
            )
    ...
    for line in array_lines:
        ...
        if form.do_not_check_dublicates.data == "0":
            found = db.select_creds_dublicates(
                current_project["id"],
                login,
                hash,
                cleartext,
        ...
        if add:
            if form.check_pwd.data != "" and hash != "" and form.hash_type.data != "":
                # TODO: add more hashes
                hash_function = ""
                if form.hash_type.data == "md5_hex":
                    hash_function = md5_hex_str
                ...
                if dict_words and hash_function != "":
                    for cleartext_pwd in dict_words:
                        if hash_function(cleartext_pwd) == hash.strip():
...
##################################
@routes.route(
    "/project/<uuid:project_id>/credentials/<uuid:creds_id>/", methods=["GET"]
)
@requires_authorization
@check_session
@check_password_reset
...
def project_credentials_info(
    project_id, current_project, current_user, creds_id, current_creds
):
    return render_template(
        "project/creds/edit.html",
        current_project=current_project,
        current_creds=current_creds,
        tab_name="User info"
        if not current_creds["login"]
        else "Login: {}".format(current_creds["login"]),
    )


@routes.route(
    "/project/<uuid:project_id>/credentials/<uuid:creds_id>/", methods=["POST"]
)
@requires_authorization
@check_session
@check_password_reset
...
@check_project_creds
@send_log_data
def project_credentials_info_form(
    project_id, current_project, current_user, creds_id, current_creds
):
...
@routes.route("/project/<uuid:project_id>/credentials/export", methods=["POST"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def project_credentials_export(project_id, current_project, current_user):
    ...
    if not form.errors:
        ...
        users_arr = list(set([current_creds["login"] for current_creds in creds_array]))
        passwords_arr = list(
        ...
        if form.export_type.data == "usernames":
            ...
            for current_creds in creds_array:
                if form.empty_passwords.data or (
                    not form.empty_passwords.data and "" != current_creds["cleartext"]
                ):
                    result.append(
                        "{}{}{}".format(
                            current_creds["login"],
            ...
            for user in users_arr:
                for password in passwords_arr:
                    result.append("{}{}{}".format(user, form.divider.data, password))
...
@routes.route("/project/<uuid:project_id>/notes/", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def project_notes(project_id, current_project, current_user):
    return render_template(
        "project/notes/index.html", current_project=current_project, tab_name="Notes"
    )


@routes.route("/project/<uuid:project_id>/print_note/<uuid:note_id>/", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
@check_note_access
@send_log_data
def project_print_note(
    project_id, current_project, current_user, note_id, current_note
):
    return render_template(
        "project/notes/print_note.html",
        current_project=current_project,
...
@routes.route("/project/<uuid:project_id>/notes/add", methods=["POST"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def new_note_form(project_id, current_project, current_user):
    ...
    if not errors:
        note_id = db.insert_new_note(
            current_project["id"], form.name.data, current_user["id"], host_id=host_id
        )
...
@routes.route("/project/<uuid:project_id>/notes/edit", methods=["POST"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def edit_note_form(project_id, current_project, current_user):
    ...
    if not errors:
        if form.action.data == "Update":
            db.update_note(form.note_id.data, form.text.data, current_project["id"])
        elif form.action.data == "Delete":
            db.delete_note(form.note_id.data, current_project["id"])
        elif form.action.data == "Rename":
            db.update_note_name(
                form.note_id.data, form.text.data, current_project["id"]
            )

    return redirect("/project/{}/notes/".format(current_project["id"]))


@routes.route("/project/<uuid:project_id>/files/", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def project_new_file_form(project_id, current_project, current_user):
    ...
    if not errors:
        for host_port in form.services.data:
            ...
            if not errors:
                if hostname_id != "0":
                    hostname_data = db.select_project_hostname(
                        current_project["id"], hostname_id
                    )
                    if (
                        not hostname_data
                        or hostname_data[0]["host_id"] != port_data["host_id"]
                    ):
...
@send_log_data
def project_file_info(project_id, current_project, current_user, file_id, current_file):
    return render_template(
        "project/files/view.html",
        current_project=current_project,
...
@routes.route(
    "/project/<uuid:project_id>/files/<uuid:file_id>/download", methods=["GET"]
)
@requires_authorization
@check_session
@check_password_reset
...
def project_file_download(
    project_id, current_project, current_user, file_id, current_file
):
...
@routes.route("/project/<uuid:project_id>/files/<uuid:file_id>/", methods=["POST"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
@check_project_file_access
@send_log_data
def project_file_edit(project_id, current_project, current_user, file_id, current_file):
    form = EditFile()
...
@check_project_access
@send_log_data
def project_settings_form(project_id, current_project, current_user) -> Response:
    # team access check
    ...
    if not (
        current_project["admin_id"] == current_user["id"] or current_user["admin"] == 1
    ):
        errors.append(
            "You Don't Have the permission to modify this project's attribuites."
        )
    ...
    if form.action.data == "Archive":
    ...
    elif form.action.data == "Update" and not errors:
        form_users = [user for user in form.users.data if user]
        if current_user["admin"] != 1:
            for i, v in {
                "admin_id": "Your Team Lead Position",
                "level": "Project Level",
                "rating": "Project Rating",
                "classification": "Project Classification",
                "project_type": "Project Type",
            }.items():
                if form.__getattribute__(i).data != current_project[i]:
                    errors.append("You are not allowed to change {}.".format(v))

        if form.admin_id.data not in form_users:  # Add team lead to Testers
            form_users = [form.admin_id.data] + form_users
        ...
        if not errors:
            ...
            project_id = db.update_project_settings(
                current_project["id"],
                form.name.data,
                description=form.description.data,
                project_type=form.project_type.data,
                scope=form.scope.data,
                out_of_scope=form.out_of_scope.data,
                start_date=start_time,
                end_date=end_time,
                auto_archive=form.archive.data,
                testers=form_users,
        ...
        current_project = db.check_user_project_access(
            current_project["id"], session["id"]
        )
    else:
        errors.append("Unknown command!")
...
def project_services(project_id, current_project, current_user):
    return render_template(
        "project/services/list.html",
        current_project=current_project,
        tab_name="Services",
    )


@routes.route("/project/<uuid:project_id>/services/new_service", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def project_new_services(project_id, current_project, current_user):
    return render_template(
        "project/services/new.html",
        current_project=current_project,
        tab_name="New Service",
    )


@routes.route("/project/<uuid:project_id>/services/new_service", methods=["POST"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def project_report_export(project_id, current_project, current_user):
    ...
    for issue in issues:
        ...
        for port_id in services:
            ...
            target["ip"] = db.select_project_host(
                current_project["id"], port["host_id"]
            )[0]["ip"]
            hostnames = []
            for hostname_id in services[port_id]:
                if hostname_id != "0":
                    hostnames += [
                        x["hostname"] for x in db.select_hostname(hostname_id)
                    ]
            target["hostnames"] = hostnames

            pocs = db.select_issue_pocs(issue["id"])
            for poc in pocs:
                poc_object = {"filename": poc["filename"], "url": "", "type": ""}
...
def project_new_chat(project_id, current_project, current_user):
    ...
    if not errors:
        chat_id = db.insert_chat(
            current_project["id"], form.name.data, current_user["id"]
        )
    return redirect("/project/{}/chats/".format(current_project["id"]))


@routes.route("/project/<uuid:project_id>/chats/<uuid:chat_id>/edit", methods=["POST"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
@check_chat_access
@send_log_data
def project_edit_chat_form(
    project_id, current_project, current_user, chat_id, current_chat
):
    form = EditChat()
    ...
    if not errors:
        if form.action.data == "delete":
            ...
            return redirect(
                "/project/{}/chats/#/chat_{}".format(
                    current_project["id"], current_chat["id"]
                )
            )
    return redirect("/project/{}/chats/".format(current_project["id"]))
...
@routes.route(
    "/project/<uuid:project_id>/chats/<uuid:chat_id>/getall.json", methods=["GET"]
)
@requires_authorization
@check_session
@check_password_reset
...
@check_chat_access
@send_log_data
def project_chats_getlastmsg(
    project_id, current_project, current_user, chat_id, current_chat, last_msg_time
):
...
@routes.route(
    "/project/<uuid:project_id>/chats/<uuid:chat_id>/sendmessage", methods=["POST"]
)
@requires_authorization
@check_session
@check_password_reset
    ...
    if form.errors:
        for field in form.errors:
            for error in form.errors[field]:
                errors.append(error)
    if not errors:
        message_time = db.insert_new_message(
            current_chat["id"], form.message.data, current_user["id"]
        )
        return str(message_time)
    return "Error!"


@routes.route("/share/issue/<uuid:issue_id>/", methods=["GET"])
def issues_info_share(issue_id):
    ...
    return render_template(
        "project/issues/share.html",
        current_issue=current_issue,
        tab_name=current_issue["name"] if current_issue["name"] else "Issue",
    )


@routes.route("/project/<uuid:project_id>/reports/", methods=["POST"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def generate_report(project_id, current_project, current_user):
    ...
    errors = []
    unix_time = time.time()
    curr_time = datetime.datetime.fromtimestamp(int(unix_time)).strftime(
        config["design"]["report_filename_date"]
    )
    if form.errors:
        for field in form.errors:
            for error in form.errors[field]:
    ...
    if not errors:
        if "file" in request.files and request.files.get("file").filename:
            ...
            file = request.files.get("file")
            template_tmp_name = gen_uuid()
            report_path = path.join(config["main"]["tmp_path"], template_tmp_name)
            ...
            if is_valid_uuid(form.template_id.data):
                if db.select_report_templates(template_id=form.template_id.data):
                    report_path = path.join(
                        "./static/files/templates/", form.template_id.data
                    )
            else:
    ...
    if not errors:
        ...
        def docx_image(image_id, width=None, height=None):
            if not is_valid_uuid(image_id):
                return None
            tmp_image_path = path.join(config["main"]["tmp_path"], image_id + ".png")
            image_object = InlineImage(template_obj, tmp_image_path)
            if width and height:
                image_object = InlineImage(
                    template_obj, tmp_image_path, width=Mm(width), height=Mm(height)
                )
            elif width:
                image_object = InlineImage(
                    template_obj, tmp_image_path, width=Mm(width)
                )
            elif height:
                image_object = InlineImage(
                    template_obj, tmp_image_path, height=Mm(height)
                )
            else:
                image_object = InlineImage(template_obj, tmp_image_path)
            return image_object
        ...
        if zipfile.is_zipfile(report_path):
            if isValidDocx(report_path):
                ...
                docx_uuid = gen_uuid()
                result_docx_path = path.join(
                    config["main"]["tmp_path"], docx_uuid + ".docx"
                )
                template_images = []
                ...
                def docx_image(image_id, width=None, height=None):
                    if not is_valid_uuid(image_id):
                        return None
                    tmp_image_path = path.join(
                        config["main"]["tmp_path"], image_id + ".png"
                    )
                    image_object = InlineImage(template_obj, tmp_image_path)
                    if width and height:
                        image_object = InlineImage(
                ...
                for poc_id in project_dict["pocs"]:
                    ...
                    if project_dict["pocs"][poc_id]["filetype"] == "image":
                        tmp_image_id = poc_id
                        tmp_image_path = path.join(
                            config["main"]["tmp_path"], tmp_image_id + ".png"
                        )
                ...
                try:
                    run_function_timeout(
                        template_obj.render,
                        int(config["timeouts"]["report_timeout"]),
                        {
                            ...
                            "functions": {
                                "format_date": lambda unix_time, str_format: datetime.datetime.fromtimestamp(
                                    int(unix_time)
                                ).strftime(
                                    str_format
                                ),
                                "docx_image": docx_image,
                                "ips_in_subnets": lambda ip_arr, network_arr: True
                                in [
                                    ipaddress.ip_address(ip)
                                    in ipaddress.ip_network(network, False)
                                    for ip in ip_arr
                ...
                return Response(
                    ...
                    headers={
                        "Content-disposition": "attachment; filename=report_{}.docx".format(
                            curr_time
                        )
                    },
                )
                ...
                zip_unpack_size = sum(
                    [zinfo.file_size for zinfo in zipfile_obj.filelist]
                )
                ...
                if "file" in request.files and request.files.get("file").filename:
                    remove(report_path)
                result_zip_path = path.join(
                    config["main"]["tmp_path"], zip_uuid + ".zip"
                )
                result_zip_obj = zipfile.ZipFile(
                                ...
                                try:
                                    if file_ext == "docx":
                                        ...
                                        if isValidDocx(report_path):
                                            ...
                                            docx_uuid = gen_uuid()
                                            result_docx_path = path.join(
                                                config["main"]["tmp_path"],
                                                docx_uuid + ".docx",
                                            )
                                            template_images = []

                                            def docx_image(
                                                image_id, width=None, height=None
                                            ):
                                                if not is_valid_uuid(image_id):
                                                    return None
                                                tmp_image_path = path.join(
                                            ...
                                            for poc_id in project_dict["pocs"]:
                                                ...
                                                if (
                                                    ...
                                                    original_image_path = path.join(
                                                        "./static/files/poc/", poc_id
                                                    )
                                                    shutil.copyfile(
                                                        original_image_path,
                                                        tmp_image_path,
                                            ...
                                            run_function_timeout(
                                                template_obj.render,
                                                int(
                                                    config["timeouts"]["report_timeout"]
                                                ),
                                                {
                                                    "project": project_dict["project"],
                                                    "issues": project_dict["issues"],
                                                    ...
                                                    "notes": project_dict["notes"],
                                                    "paths": project_dict["paths"],
                                                    "networks": project_dict[
                                                        "networks"
                                                    ],
                                                    "credentials": project_dict[
                                                        "credentials"
                                                    ],
                                                    "functions": {
                                                        "format_date": lambda unix_time, str_format: datetime.datetime.fromtimestamp(
                ...
                for current_poc in db.select_project_pocs(current_project["id"]):
                    poc_server_path = path.join(
                        path.join("./static/files/poc/", current_poc["id"])
                    )
                    if current_poc["type"] == "text":
                        poc_save_path = path.join(
                            poc_save_dir, current_poc["id"] + ".txt"
                        )
                    elif current_poc["type"] == "image":
                        poc_save_path = path.join(
                            poc_save_dir, current_poc["id"] + ".png"
                        )
                ...
                return Response(
                    ...
                    headers={
                        "Content-disposition": "attachment; filename=report_{}.zip".format(
                            curr_time
                        )
                    },
                )
            ...
            try:
                ...
                rendered_txt = run_function_timeout(
                    ...
                    functions={
                        ...
                        "latex_escape": latex_str_escape,
                        "ip_in_subnet": lambda ip, network: ipaddress.ip_address(ip)
                        in ipaddress.ip_network(network, False),
                    },
                )
                if rendered_txt:
                    ...
                    return Response(
                        ...
                        mimetype="text/plain",
                        headers={
                            "Content-disposition": "attachment; filename=report_{}.txt".format(
                                curr_time
                            )
                        },
                    )
...
@routes.route("/project/<uuid:project_id>/services/edit", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def project_edit_service_form(project_id, current_project, current_user):
    ...
    current_service = db.select_ports_by_fields(
        current_project["id"], port, is_tcp, service, info
    )
...
@routes.route("/project/<uuid:project_id>/services/edit", methods=["POST"])
@requires_authorization
@check_session
@check_password_reset
@check_project_access
...
def project_edit_service(project_id, current_project, current_user):
    ...
    if not errors:
        db.update_service_multiple_info(
            current_project["id"],
            old_port,
            old_is_tcp,
            form.old_service.data,
        ...
        return redirect(
            "/project/{}/services/edit?{}".format(current_project["id"], url_get_str)
        )
...
@routes.route(
    "/project/<uuid:project_id>/issue_template/<uuid:template_id>/", methods=["GET"]
)
@requires_authorization
@check_session
@check_password_reset
...
@check_issue_template_access
@send_log_data
def project_create_issue_from_template(
    project_id, current_project, current_user, template_id, current_template
):
    return render_template(
        "project/issues/from_template.html",
        current_project=current_project,
        tab_name="New issue",
        current_template=current_template,
    )
...
@routes.route(
    "/project/<uuid:project_id>/issue_template/<uuid:template_id>/", methods=["POST"]
)
@requires_authorization
@check_session
@check_password_reset
...
@check_issue_template_access
@send_log_data
def project_create_issue_from_template_form(
    project_id, current_project, current_user, template_id, current_template
):
        ...
        if not add:
            ...
            if not errors:
                ...
                if host_id:
                    ...
                    db.update_host_comment_threats(host_id, description, threats, os)
                else:
                ...
                if host_id and hostname:
                    ...
                    if hostname_id:
                        ...
                        hostname_id = db.insert_hostname(
                            host_id, hostname, "", current_user["id"]
                        )
...
def project_issue_rules_json(project_id, current_project, current_user):
    rules_list = db.select_user_all_issue_rules(
        current_user["id"], current_user["email"]
    )

    rules_json = [
        {"name": x["name"], "id": x["id"], "owner_name": str(x["owner_name"])}
        for x in rules_list
    ]
...
def project_issue_rules_form(project_id, current_project, current_user):
    ...
    # check rules access:
    rules_obj_list = db.check_user_issue_rules_access(
        rules_list, current_user["id"], current_user["email"]
    )
            ...
            for search_rule in current_rule["search_rules"]:
                ...
                if field_name == "name":
                    ...
                    for issue_field_name in current_issue["fields"]:
                        if (
                            issue_field_name == new_name
                            and current_issue["fields"][issue_field_name]["type"]
                            != "file"
                        ):
                            compare_str = str(
                                current_issue["fields"][issue_field_name]["value"]
                            )
                # check
                ...
                try:
                    found = bool(
                        run_function_timeout(
                            re.match,
                            int(config["timeouts"]["regexp_timeout"]),
                            pattern=reg_exp,
                            string=compare_str,
                        )
                    )
                    ...
                    if field_name == "name":
                        ...
                        for issue_field_name in current_issue["fields"]:
                            if (
                                ...
                                compare_str = str(
                                    current_issue["fields"][issue_field_name]["val"]
                                )
                    ...
                    if replace_rule["type"] != "template":
                        ...
                        if field_name in [
                            ...
                            for issue_field_name in current_issue["fields"]:
                                if (
                                    issue_field_name == new_name
                                    and current_issue["fields"][issue_field_name][
                                        "type"
                                    ]
                                    != "file"
                                ):
                                    found = True
                                    field_original_value = str(
                                        current_issue["fields"][issue_field_name]["val"]
                                    )
                                    special_var = True
                        ...
                        if found:
                            ...
                            if special_var:
                                # additional field
                                field_type = current_issue["fields"][
                                    field_name.strip("_")
                                ]["type"]
                                try:
                                    if field_type == "text":
                                        current_issue["fields"][field_name.strip("_")][
                                            "val"
                                        ] = str(result_string)
                                    elif field_type == "float":
                                        current_issue["fields"][field_name.strip("_")][
                        ...
                        current_template["variables"] = json.loads(
                        ...
                        for field_name in current_template["variables"]:
                            if field_name in replace_rule["vars"]:
                                try:
                                    if (
                                        current_template["variables"][field_name][
                                            "type"
                                        ]
                                        == "boolean"
                                    ):
                                        current_template["variables"][field_name][
                                            "value"
                                        ] = bool(replace_rule["vars"][field_name])
                                    elif (
                                        current_template["variables"][field_name][
                                            "type"
                                        ]
                                        ...
                                        current_template["variables"][field_name][
                                            "value"
                                        ] = int(replace_rule["vars"][field_name])
                                    elif (
                                        current_template["variables"][field_name][
                                            "type"
                                        ]
                                        ...
                                        current_template["variables"][field_name][
                                            "value"
                                        ] = float(replace_rule["vars"][field_name])
                                    elif (
                                        current_template["variables"][field_name][
                                            "type"
                                        ]
                                        ...
                                        current_template["variables"][field_name][
                                            "value"
                                        ] = str(replace_rule["vars"][field_name])
                                except Exception as e:
                                    pass
                        ...
                        def replace_tpl_text(text: str):
                            for variable_name in add_variables_dict:
                                variable_type = add_variables_dict[variable_name][
                                    "type"
                                ]
                                variable_value = add_variables_dict[variable_name][
                                    "value"
                                ]
                                if variable_type == "boolean":
                                    variable_value = int(variable_value)
                                text = text.replace(
                                    "__" + variable_name + "__", str(variable_value)
                                )
                            return text

                        issue_name = replace_tpl_text(current_template["name"])
                        issue_description = replace_tpl_text(
