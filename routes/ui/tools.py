...
from system.crypto_functions import gen_uuid, base64
...
def project_tools(project_id, current_project, current_user):
    return render_template(
        "project/tools/list.html", current_project=current_project, tab_name="Tools"
    )
...
def nmap_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/nmap.html",
...
def nmap_page_form(project_id, current_project, current_user):
    ...
    if not errors:
        ...
        ignore_services_array = [
            service.lower()
            for service in form.ignore_services.data.replace(" ", "").split(",")
        ]
        ...
        for file in form.files.data:
            ...
            for host in nmap_report.hosts:
                ...
                for service in host.services:
                    ...
                    if (
                        [port_num, protocol] not in ignore_port_array
                        and service_name not in ignore_services_array
                    ):
                        if service.state == "open":
                            found = 1
                        elif service.state == "filtered" and form.rule.data in [
                            "filtered",
                            "closed",
                        ]:
                            found = 1
                ...
                if found or add_empty_hosts:
                    ...
                    for service in host.services:
                        ...
                        if [
                            port_num,
                            protocol_str,
                        ] not in ignore_port_array and service_name.lower() not in ignore_services_array:
                            if service.state == "open":
                                add = 1
                            elif service.state == "filtered" and form.rule.data in [
                                "filtered",
                                ...
                                service.state == "closed" and form.rule.data == "closed"
                            ):
                                add = 1
                                service_banner += "\nstate: closed"
                        if add == 1:
                            port_id = db.select_ip_port(host_id, service.port, is_tcp)
                            if not port_id:
                                port_id = db.insert_host_port(
                                    host_id,
                                    service.port,
                                    is_tcp,
                                    service_name,
                                ...
                                db.update_port_proto_description(
                                    port_id, service_name, service_banner
                                )
                            ...
                            for script_xml in service.scripts_results:
                                for script in nmap_scripts.modules:
                                    ...
                                    if (
                                        script_class.script_id == script_xml["id"]
                                        and script_class.script_source == "service"
                                    ):
                                        ...
                                        if "port_info" in script_obj.script_types:
                                            ...
                                            if (
                                                "info" in result
                                                and result["info"]
                                                and result["info"].lower()
                                                not in service_banner.lower()
                                            ):
                                                service_banner += "\n" + result["info"]
                                                update = True
                                            if update:
                                        ...
                                        if "issue" in script_obj.script_types:
                                            ...
                                            for issue in issues:
                                                db.insert_new_issue_no_dublicate(
                                                    issue["name"],
                                                    issue["description"]
                                        ...
                                        if "credentials" in script_obj.script_types:
                                            ...
                                            for cred in credentials:
                                                ...
                                                if dublicates_creds:
                                                    dublicates_creds = dublicates_creds[
                                                        0
                                                    ]
                                                    services = json.loads(
                                                        dublicates_creds["services"]
                                                    )
                    ...
                    for script_xml in host.scripts_results:
                        for script in nmap_scripts.modules:
                            ...
                            if (
                                script_class.script_id == script_xml["id"]
                                and script_class.script_source == "host"
                            ):
                                script_obj = script_class(script_xml)
                                ...
                                if "server_info" in script_obj.script_types:
                                    ...
                                    if (
                                        ...
                                        and result["info"]
                                        and result["info"].lower()
                                        not in current_host["comment"].lower()
                                    ):
                                        current_host["comment"] += "\n" + result["info"]
                                        update = True
                                    if update:
                                        db.update_host_comment_threats(
                                            current_host["id"],
                                            current_host["comment"],
                                    ...
                                    if "hostnames" in result:
                                        for hostname in result["hostnames"]:
                                            hostnames_found = db.select_ip_hostname(
                                                current_host["id"], hostname
                                            )
                                ...
                                if "credentials" in script_obj.script_types:
                                    ...
                                    for cred in credentials:
                                        ...
                                        hash_str = (
                                            cred["hash"] if "hash" in cred else ""
                                        )
                                        description = (
                                            cred["description"]
                                            if "description" in cred
                                        ...
                                        source = (
                                            cred["source"] if "source" in cred else ""
                                        )

                                        dublicates_creds = db.select_creds_dublicates(
                                            current_project["id"],
                                            login,
                                            hash_str,
                                            cleartext,
                                        ...
                                        if dublicates_creds:
                                            ...
                                            if host_zero_port["id"] not in services:
                                                ...
                                                services[host_zero_port["id"]].append(
                                                    "0"
                                                )
...
def nessus_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/nessus.html",
        current_project=current_project,
        tab_name="Nessus",
    )
    ...
    if not errors:
        ...
        for file in form.xml_files.data:
            if file.filename:
                ...
                found_hosts = {
                    x["ip"]: {"id": x["id"], "os": x["os"]}
                    for x in db.select_project_hosts_multiple_ip(
                        current_project["id"], ip_list
                    )
                }
                ...
                hostnames_list = {
                    host_obj.ip: host_obj.name if host_obj.name != host_obj.ip else ""
                    for host_obj in scan_result.hosts
                }

                hostnames_found = {
                    x["id"]: {"hostname": x["hostname"], "ip": x["ip"]}
                    for x in db.select_hostnames_ip_multiple(
                        current_project["id"],
                ...
                for hostname_ip in hostnames_list:
                    ...
                    if hostname:
                        ...
                        for found_hostname_id in hostnames_found:
                            found_hostname = hostnames_found[found_hostname_id][
                                "hostname"
                            ]
                ...
                for host in scan_result.hosts:
                    ...
                    for issue in host.get_report_items:
                        if issue.get_vuln_plugin["pluginName"] == "OS Identification":
                            os = (
                                issue.get_vuln_plugin["plugin_output"]
                                .split("\n")[1]
                                .split(" : ")[1]
                            )
                ...
                db.update_host_os_multiple(
                    [x for x in os_list], [os_list[x] for x in os_list]
                )
                ...
                for host in scan_result.hosts:
                    ...
                    for issue in host.get_report_items:
                        ...
                        for exists_port in ports_list[host_id]:
                            if (
                                exists_port["is_tcp"] == is_tcp
                                and exists_port["port_num"] == port_num
                            ):
                                found = 1
                        if not found:
                ...
                ports_found = db.select_project_ports_multiple(
                    current_project["id"], port_num_list, is_tcp_list, host_id_list
                )
                not_found_ports = {}
                for host_id in ports_list:
                    for port_js in ports_list[host_id]:
                        ...
                        for port_obj in ports_found:
                            if (
                                port_obj["is_tcp"] == port_js["is_tcp"]
                                and port_obj["host_id"] == host_id
                                and port_obj["port"] == port_js["port_num"]
                            ):
                                found = 1
                        if not found:
                            if host_id not in not_found_ports:
                                ...
                                for port_obj1 in not_found_ports[host_id]:
                                    try:
                                        if (
                                            port_obj1["port_num"] == port_js["port_num"]
                                            and port_obj1["is_tcp"] == port_js["is_tcp"]
                                        ):
                                            found = 1
                                    except Exception as e:
                                        pass
                ...
                for port_obj in ports_found:
                    ...
                    for host_id in ports_list:
                        for port_js in ports_list[host_id]:
                            if (
                                port_js["port_num"] == port_obj["port"]
                                and port_js["is_tcp"] == port_obj["is_tcp"]
                                and host_id == port_obj["host_id"]
                            ):
                                update_port_services_list.append(port_js["service"])

                db.update_port_service_multiple(
                    update_port_ids_list, update_port_services_list
                )
                ...
                for host in scan_result.hosts:
                    ...
                    if issue_hostname:
                        for hostname_id in all_hostnames:
                            if (
                                all_hostnames[hostname_id]["ip"] == issue_ip
                                and all_hostnames[hostname_id]["hostname"]
                                == issue_hostname
                            ):
                    ...
                    for issue in host.get_report_items:
                        ...
                        issue_description = "Plugin name: {}\r\n\r\nInfo: \r\n{} \r\n\r\nOutput: \r\n {}".format(
                            issue.plugin_name, issue_info, issue.description.strip("\n")
                        )
                        issue_cve = (
                            issue.cve.replace("[", "")
                            .replace("]", "")
                        ...
                        if (
                            issue_severity == 0
                            and issue.get_vuln_info["risk_factor"] == "None"
                        ):
                            issue_cvss = 0
                        ...
                        for port_db in all_ports:
                            if (
                                port_db["port"] == issue_port
                                and port_db["is_tcp"] == issue_is_tcp
                                and port_db["host_id"] == issue_host_id
                            ):
                                issue_port_id = port_db["id"]

                        if issue_severity > 0 or (
                            issue_severity == 0 and add_info_issues
                        ):
                            ...
                            for exist_issue in db_issues:
                                if (
                                    not found
                                    and exist_issue["name"] == issue_name
                                    and exist_issue["description"] == issue_description
                                    and exist_issue["url_path"] == issue_url_path
                                    and exist_issue["cvss"] == issue_cvss
                                    and exist_issue["status"] == issue_status
                                    and exist_issue["cve"] == issue_cve
                                    and exist_issue["cwe"] == issue_cwe
                                    and exist_issue["type"] == issue_type
                                    and exist_issue["fix"] == issue_fix
                                    and exist_issue["param"] == issue_param
                                ):
                                    found = 1
                                    ...
                                    if exist_issue["id"] in issues_update_services:
                                        ...
                                        issues_update_services[
                                            exist_issue["id"]
                                        ] = json.loads(exist_issue["services"])
                                        if (
                                            issue_port_id
                                            not in issues_update_services[
                                                exist_issue["id"]
                                            ]
                                        ):
                                            issues_update_services[exist_issue["id"]][
                                                issue_port_id
                                            ] = [issue_hostname_id]
                                        else:
                                            if (
                            ...
                            if not found:
                                ...
                                for new_issue in issue_create_list:
                                    if (
                                        not found
                                        and new_issue["name"] == issue_name
                                        and new_issue["description"]
                                        == issue_description
                                        and new_issue["url_path"] == issue_url_path
                                        and new_issue["cvss"] == issue_cvss
                                        and new_issue["status"] == issue_status
                                        and new_issue["cve"] == issue_cve
                                        and new_issue["cwe"] == issue_cwe
                                        and new_issue["type"] == issue_type
                                        and new_issue["fix"] == issue_fix
                                        and new_issue["param"] == issue_param
                                    ):
                                        found = 1
                                        ...
                                        if issue_port_id not in old_services:
                                            old_services[issue_port_id] = [
                                                issue_hostname_id
                                            ]
                                        else:
                                            if (
                                                issue_hostname_id
                                                not in old_services[issue_port_id]
                                            ):
                                                old_services[issue_port_id].append(
                                                    issue_hostname_id
                                                )
                                            else:
                                                pass  # not changed
                                ...
                                issue_create_list.append(
                                    {
                                        ...
                                        "param": issue_param,
                                        "services": {
                                            issue_port_id: [issue_hostname_id]
                                        },
                ...
                db.update_issue_services_multiple(
                    ...
                    [
                        issues_update_services[issue_id]
                        for issue_id in issues_update_services
                    ],
                )
...
def nikto_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/nikto.html",
        current_project=current_project,
        tab_name="Nikto",
    )
...
def nikto_page_form(project_id, current_project, current_user):
    ...
    if not errors:
        ...
        for file in form.json_files.data:
            if file.filename:
                json_report_data = (
                    file.read().decode("charmap").replace(",]", "]").replace(",}", "}")
                )
                scan_result = json.loads(json_report_data)
                host = scan_result["ip"]
                hostname = (
                    scan_result["host"]
                    if scan_result["ip"] != scan_result["host"]
                    else ""
                )
                ...
                if scan_result["banner"]:
                    port_description = "Nikto banner: {}".format(scan_result["banner"])
        ...
        for file in form.csv_files.data:
            if file.filename:
                scan_result = csv.reader(
                    codecs.iterdecode(file, "charmap"), delimiter=","
                )
...
def acunetix_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/acunetix.html",
        current_project=current_project,
        tab_name="Acunetix",
    )
...
def acunetix_page_form(project_id, current_project, current_user):
    ...
    if not errors:
        ...
        for file in form.files.data:
            if file.filename:
                scan_result = BeautifulSoup(file.read(), "html.parser").scangroup.scan
                ...
                port_description = "Banner: {} Web: {}".format(port_banner, web_banner)
                ...
                if not errors:
                    ...
                    if not port_id:
                        ...
                        port_id = port_id[0]["id"]
                        db.update_port_proto_description(
                            port_id, protocol, port_description
                        )
...
def exporter_page_form(project_id, current_project, current_user):
    ...
    if not errors:
        result_hosts = db.search_hostlist(
    ...
    result = ""
    separator = "\n" if form.separator.data == "[newline]" else form.separator.data
    host_export = form.hosts_export.data

    ports_array = []
    if form.port.data:
        ports_array = [
            [int(port.split("/")[0]), port.split("/")[1] == "tcp"]
    ...
    if form.filetype.data == "txt":
        ...
        if not form.add_ports.data:
            ...
            for host in result_hosts:
                ...
                hostnames += [hostname["hostname"] for hostname in host_hostname]
            ...
            if host_export == "ip":
                ...
                for ip in ips_hostnames:
                    if not ips_hostnames[ip]:
                        ...
                        host_hostnames_arr += [
                            hostname["hostname"] for hostname in ips_hostnames[ip]
                        ]
                result = separator.join(
                    [prefix + x + postfix for x in host_hostnames_arr]
                )
        else:
            # with ports
            ...
            if form.issue_name.data:
                port_ids = db.search_issues_port_ids(
                    current_project["id"], form.issue_name.data
                )
            ...
            for host in result_hosts:
                ...
                for port in ports:
                    if (not form.port.data) or (
                        ...
                        if (
                            form.service.data == ""
                            or form.service.data == port["service"]
                        ):
                            if (not form.issue_name.data) or (port["id"] in port_ids):
        ...
        for host in result_hosts:
            ...
            host_object["hostnames"] = [hostname["hostname"] for hostname in hostnames]
            host_object["ports"] = []
            for port in ports:
                if (not form.port.data) or (
                    [port["port"], port["is_tcp"]] in ports_array
            ...
            if not (
                (not host_object["ports"])
                and (form.port.data or form.service.data or form.issue_name.data)
            ):
    ...
    if form.open_in_browser.data:
        ...
        return send_file(
            ...
            attachment_filename="{}.{}".format(form.filename.data, form.filetype.data),
...
def checkmarx_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/checkmarx.html",
        current_project=current_project,
        tab_name="Checkmarx",
    )
        ...
        for file in form.xml_files.data:
            if file.filename:
                ...
                for query in query_list:
                    vulnerability_name = re.sub(
                        " Version:[0-9]+", "", query.attrs["querypath"].split("\\")[-1]
                    )
...
def depcheck_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/depcheck.html",
        current_project=current_project,
        tab_name="DepCheck",
    )
                    ...
                    for vuln_example in vuln_array:
                        ...
                        for path_obj in soft_search:
                            ...
                            if "versionstartincluding" in path_obj.attrs:
                                versions += (
                                    str(path_obj.attrs["versionstartincluding"]) + "<=x"
                                )
                            if "versionstartexcluding" in path_obj.attrs:
                                versions += (
                                    str(path_obj.attrs["versionendexcluding"]) + "<x"
                                )
                            if "versionendincluding" in path_obj.attrs:
                                versions += "<=" + str(
                                    path_obj.attrs["versionendincluding"]
                                )
                            if "versionendexcluding" in path_obj.attrs:
                                versions += "<" + str(
                                    path_obj.attrs["versionendexcluding"]
                                )
...
def openvas_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/openvas.html",
        current_project=current_project,
        tab_name="OpenVAS",
    )
        ...
        for file in form.xml_files.data:
            if file.filename:
                ...
                for query in query_list:
                    if query.find(
                        "host"
                    ):  # disables result tags inside issue description
                        ...
                        issue_is_tcp = int(
                            query.find("port").text.split("/")[1] == "tcp"
                        )
                        ...
                        if links_list:
                            full_description += "\n\nLinks:\n" + "\n".join(links_list)
                        services = {port_id: [hostname_id] if hostname_id else ["0"]}
                        db.insert_new_issue_no_dublicate(
                            issue_name,
                            full_description,
                            "",
...
def netsparker_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/netsparker.html",
        current_project=current_project,
        tab_name="NetSparker",
    )
...
def netsparker_page_form(project_id, current_project, current_user):
    def beautify_output(xml_str):
        ...
        xml_str = xml_str.replace("<a target='_blank' href='", "").replace(
            ...
            if file.filename:
                ...
                for vuln in query_list:
                    ...
                    if is_confirmed or (not form.only_confirmed):
                        ...
                        vuln_severity = vuln.find(
                            "severity"
                        ).text  # High, Medium, Low, Information, BestPractice
                        vuln_description = beautify_output(
                            vuln.find("description").text
                        )
                        ...
                        vuln_skills = beautify_output(
                            vuln.find("requiredskillsforexploitation").text
                        )
                        vuln_links = beautify_output(
                            vuln.find("externalreferences").text
                        )
                        vuln_fix1_links = beautify_output(
                            vuln.find("remedyreferences").text
                        )
                        ...
                        for info_obj in info_list:
                            ...
                            if info_name == "ParserAnalyzerEngine_InputName":
                                ...
                                info_name == "ParserAnalyzerEngine_IdentifiedFieldName"
                            ):
                                vuln_args += ", (Input) " + info_name
                        ...
                        if classification_obj.find("cvss"):
                            for cvss_obj in classification_obj.find("cvss").findAll(
                                "score"
                            ):
                        ...
                        if not vuln_host and vuln_hostname:
                            try:
                                vuln_host = str(
                                    socket.gethostbyname(vuln_host_unverified)
                                )
                            except:
                                pass
                        ...
                        host_id = ""
                        if vuln_host:
                            dublicate_host = db.select_project_host_by_ip(
                                current_project["id"], vuln_host
                            )
                            ...
                            dublicate_port = db.select_host_port(
                                host_id, vuln_port, True
                            )
                            ...
                            if vuln_hostname:
                                dublicate_hostname = db.select_ip_hostname(
                                    host_id, vuln_hostname
                                )
...
def qualys_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/qualys.html",
        current_project=current_project,
        tab_name="Qualys",
    )
                ...
                for host in hosts_list:
                    ...
                    if ports_list:
                        for port_obj in ports_list.findAll("cat"):
                            if (
                                "port" in port_obj.attrs
                                and "protocol" in port_obj.attrs
                            ):
                                if not host_id:
                                    host_id = db.insert_host(
                                        current_project["id"],
                    ...
                    if issues_list:
                        for issue_obj in issues_list.findAll("cat"):
                            ...
                            is_tcp = 1
                            if (
                                "port" in issue_obj.attrs
                                and "protocol" in issue_obj.attrs
                            ):
                            ...
                            issue_solution = beautify_output(
                                issue_obj.find("solution").text
                            )
                            ...
                            issue_full_description = (
                                "Diagnosis: \n{} \n\nConsequence: \n{}".format(
                                    issue_diagnostic, issue_description
                                )
                            )
                            issue_full_description = beautify_output(
                                issue_full_description
                            )
                            services = {port_id: ["0"]}
                            issue_id = db.insert_new_issue_no_dublicate(
                    ...
                    if issues_list:
                        for issue_obj in issues_list.findAll("practice"):
                            ...
                            if " detected on port " in info_str:
                                port = int(
                                    info_str.split(" detected on port ")[1]
                                    .split(" ")[0]
                                    .split(".")[0]
                                )
...
def duplicator_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/export/duplicator.html",
        current_project=current_project,
        tab_name="Duplicator",
    )
...
def duplicator_page_form(project_id, current_project, current_user):
    ...
    destination_project = db.check_user_project_access(
        form.destination_project.data, current_user["id"]
    )
    ...
    if not errors:
        if not (
            destination_project["status"]
            and not (
                destination_project["auto_archive"]
                and destination_project["end_date"] < time.time()
            )
        ):
    ...
    if not errors:
        ...
        if form.copy_deadline.data:
            ...
            if int(destination_project["end_date"]) < int(
        ...
        for path_id in paths_ids_list:
            current_path = db.select_path(
                path_id=path_id, project_id=current_project["id"]
            )
            if current_path:
                current_path = current_path[0]
                if (
                    current_path["host_out"]
                    and current_path["host_out"] not in hosts_ids_list
                ):
                    hosts_ids_list.append(current_path["host_out"])
                if (
                    current_path["host_in"]
                    and current_path["host_in"] not in hosts_ids_list
                ):
                    hosts_ids_list.append(current_path["host_in"])
                if (
                    current_path["network_in"]
                    and current_path["network_in"] not in networks_ids_list
                ):
                    networks_ids_list.append(current_path["network_in"])
                if (
                    current_path["network_out"]
                    and current_path["network_out"] not in networks_ids_list
                ):
        ...
        for host_id in hosts_ids_list:
            ...
            if current_host:
                ...
                # if same host exists
                destination_host = db.select_project_host_by_ip(
                    destination_project["id"], current_host["ip"]
                )
                if destination_host:
                    ...
                    destination_host_id = db.insert_host(
                        destination_project["id"],
                        current_host["ip"],
                        current_user["id"],
                    )
                ...
                for current_port in current_ports:
                    # check if port exists
                    destination_port = db.select_host_port(
                        destination_host_id,
                    ...
                    if destination_port:
                        ...
                        destination_port_id = db.insert_host_port(
                            destination_host_id,
                            int(current_port["port"]),
                            bool(current_port["is_tcp"]),
                            "",
                ...
                for current_hostname in current_hostnames:
                    ...
                    destination_hostname = db.select_ip_hostname(
                        destination_host_id, current_hostname["hostname"]
                    )
                    if destination_hostname:
                        destination_hostname_id = destination_hostname[0]["id"]
                        db.update_hostname(
                            destination_hostname_id, current_hostname["description"]
                        )
                    else:
                        hostname_id = db.insert_hostname(
                            destination_host_id,
        ...
        for issue_id in form.issues.data:
            ...
            if (
                current_issue
                and current_issue[0]["project_id"] == current_project["id"]
            ):
                ...
                for current_port_id in current_ports_dict:
                    current_port = db.select_project_port(
                        current_project["id"], current_port_id
                    )
                    if current_port:
                        current_port = current_port[0]
                        current_host = db.select_project_host(
                            current_project["id"], current_port["host_id"]
                        )
                        if current_host:
                            current_host = current_host[0]
                            destination_host = db.select_project_host_by_ip(
                            ...
                            if destination_host:
                                ...
                                if destination_port:
                                    ...
                                    # hostname search
                                    for current_hostname_id in current_ports_dict[
                                        current_port_id
                                    ]:
                                        if current_hostname_id == "0":
                                            if (
                                                destination_port["id"]
                                                not in destination_ports_dict
                                            ):
                                                destination_ports_dict[
                                                    destination_port["id"]
                                                ] = [current_hostname_id]
                                                ...
                                                destination_ports_dict[
                                                    destination_port["id"]
                                                ].append(current_hostname_id)
                                        else:
                                            current_hostname = db.select_hostname(
                                                current_hostname_id
                                            )
                                            if (
                                                current_hostname
                                                and current_hostname[0]["host_id"]
                                                == current_port["host_id"]
                                            ):
                                                current_hostname = current_hostname[0]
                                                destination_hostname = (
                                                    db.select_ip_hostname(
                                    ...
                                    destination_host_port_id = db.select_host_port(
                                        destination_host["id"]
                                    )[0]["id"]
                                    if (
                                        destination_host_port_id
                                        not in destination_ports_dict
                                    ):
                                        destination_ports_dict[
                                            destination_host_port_id
                ...
                for current_poc in current_pocs:
                    current_poc_path = path.join(
                        "./static/files/poc/", current_poc["id"]
                    )
                    destination_poc_id = gen_uuid()
                    destination_poc_path = path.join(
                        "./static/files/poc/", destination_poc_id
                    )
                    if current_poc["port_id"] == "0":
                        ...
                        current_port = db.select_project_port(
                            current_project["id"], current_poc["port_id"]
                        )
                        if current_port:
                            current_port = current_port[0]
                            current_host = db.select_project_host(
                                ...
                                if destination_host:
                                    ...
                                    if destination_port:
                                        ...
                                        if current_poc["hostname_id"] == "0":
                                            # add poc with port
                                            copyfile(
                                                current_poc_path, destination_poc_path
                                            )

                                            file_data = b""

                                            if (
                                                config["files"]["poc_storage"]
                                                == "database"
                                            ):
                                                f = open(destination_poc_path, "rb")
                                                file_data = f.read()
        ...
        for current_file_id in form.files.data:
            ...
            if current_file and current_file[0]["project_id"] == current_project["id"]:
                ...
                current_file_path = path.join(
                    "./static/files/code/", current_file["id"]
                )
                destination_file_id = gen_uuid()
                destination_file_path = path.join(
                    "./static/files/code/", destination_file_id
                )
                            ...
                            if destination_host:
                                ...
                                if destination_port:
                                    ...
                                    if (
                                        ...
                                        pass
                # check duplicates
                duplicate_network = db.select_network_by_ip(
                    destination_project["id"],
                    current_network["ip"],
        ...
        for host_id in form.note_hosts.data:
            ...
            for current_note in current_host_notes:
                current_host = db.select_project_host(
                    current_project["id"], current_note["host_id"]
                )
                if current_host:
                    current_host = current_host[0]
                    destination_host = db.select_project_host_by_ip(
        ...
        for path_id in paths_ids_list:
            ...
            if current_path:
                ...
                # search dublicates
                dublicate_paths = db.search_path(
...
def wpscan_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/wpscan.html",
        current_project=current_project,
        tab_name="WPScan",
    )
            ...
            if file.filename:
                ...
                try:
                    ...
                    current_host = db.select_project_host_by_ip(
                        current_project["id"], current_ip
                    )
                    if current_host:
                        current_host_id = current_host[0]["id"]
                    else:
                        current_host_id = db.insert_host(
                            current_project["id"],
                            current_ip,
                            current_user["id"],
                            "Added from WPScan",
                    ...
                    # create port
                    current_port_obj = db.select_host_port(
                        current_host_id, current_port_num, True
                    )
                    if current_port_obj:
                        current_port_id = current_port_obj[0]["id"]
                    else:
                        current_port_id = db.insert_host_port(
                            current_host_id,
                            current_port_num,
                            True,
                            current_scheme,
                    ...
                    if hostname == current_ip:
                        ...
                        current_hostname = db.select_ip_hostname(
                    ...
                    if interest_obj:
                        ...
                        for find_obj in interest_obj:
                            note_output += (
                                "<h2><b>URL:</b> " + find_obj["url"] + "</h2></br>"
                            )
                            note_output += "<b>Type:</b> " + find_obj["type"] + "</br>"
                            note_output += (
                                "<b>Description:</b> " + find_obj["to_s"] + "</br>"
                            )
                            note_output += (
                                "<b>Found by:</b> " + find_obj["found_by"] + "</br>"
                            )
                    ...
                    if version_obj:
                        ...
                        note_output += (
                            "<b>Version:</b> " + version_obj["number"] + "</br>"
                        )
                        note_output += (
                            "<b>Found by:</b> " + version_obj["found_by"] + "</br>"
                        )
                        note_output += "<b>Interesting entries:</b> <ol>"
                        ...
                        for current_issue in version_obj["vulnerabilities"]:
                            ...
                            issue_description = (
                                "{}\n\nURLs:\n{}\n\nwpvulndb: {}".format(
                                    ...
                                    "\n".join(
                                        [
                                            " - " + x
                                            for x in current_issue["references"]["url"]
                                        ]
                                    ),
                            ...
                            if "exploitdb" in current_issue:
                                issue_description += "\n\nExploitDB: {}".format(
                                    current_issue["exploitdb"]
                                )
                            if "youtube" in current_issue:
                                issue_description += "\n\nYoutube: {}".format(
                                    current_issue["youtube"]
                                )
                    ...
                    if main_theme_obj:
                        ...
                        note_output += (
                            "<b>Name:</b> " + main_theme_obj["slug"] + "</br>"
                        )
                        note_output += (
                            "<b>Location:</b> " + main_theme_obj["location"] + "</br>"
                        )
                        if "readme_url" in main_theme_obj:
                            note_output += (
                                "<b>Readme URL:</b> "
                                + main_theme_obj["readme_url"]
                                + "</br>"
                            )
                        if "style_uri" in main_theme_obj:
                            note_output += (
                                "<b>Official URL:</b> "
                                + main_theme_obj["style_uri"]
                                + "</br>"
                            )
                        if "version" in main_theme_obj and main_theme_obj["version"]:
                            note_output += (
                                "<b>Version:</b> "
                                + main_theme_obj["version"]["number"]
                                + "</br>"
                            )

                            note_output += "<b>Interesting entries:</b> <ol>"
                            for entry in main_theme_obj["version"][
                                "interesting_entries"
                            ]:
                                note_output += (
                                    "<li>" + htmlspecialchars(entry) + "</li>"
                        ...
                        for current_issue in main_theme_obj["vulnerabilities"]:
                            issue_name = current_issue["title"]
                            issue_fix = "Upgrade main theme {} to version >= {}".format(
                                main_theme_obj["slug"], current_issue["fixed_in"]
                            )
                            issue_cve = ",".join(current_issue["references"]["cve"])
                            issue_description = (
                                "{}\n\nURLs:\n{}\n\nwpvulndb: {}".format(
                    ...
                    if plugins_obj:
                        ...
                        for plugin_name in plugins_obj:
                            ...
                            note_output += (
                                "<b>Location:</b> " + plugin_obj["location"] + "</br>"
                            )
                            note_output += (
                                "<b>Found by:</b> " + plugin_obj["found_by"] + "</br>"
                            )
                            if (
                                "error_log_url" in plugins_obj
                                and plugin_obj["error_log_url"]
                            ):
                                note_output += (
                                    "<b>Error log URL:</b> "
                                    + plugin_obj["error_log_url"]
                                    + "</br>"
                                )
                            if (
                                "directory_listing" in plugin_obj
                                and plugin_obj["directory_listing"]
                            ):
                                note_output += (
                                    "<b>Dir listing URL:</b> "
                                    + plugin_obj["directory_listing"]
                                    + "</br>"
                                )
                            if (
                                "changelog_url" in plugin_obj
                                and plugin_obj["changelog_url"]
                            ):
                                note_output += (
                                    "<b>Changelog URL:</b> "
                                    + plugin_obj["changelog_url"]
                                    + "</br>"
                                )
                            if "readme_url" in plugin_obj and plugin_obj["readme_url"]:
                                note_output += (
                                    "<b>Readme URL:</b> "
                                    + plugin_obj["readme_url"]
                                    + "</br>"
                                )
                            note_output += "<b>Interesting entries:</b> <ol>"
                            ...
                            if "version" in plugin_obj and plugin_obj["version"]:
                                note_output += (
                                    "<b>Version:</b> "
                                    + plugin_obj["version"]["number"]
                                    + "</br>"
                                )
                                note_output += "<b>Version entries:</b> <ol>"
                                for entry in plugin_obj["version"][
                                    "interesting_entries"
                                ]:
                                    note_output += (
                                        "<li>" + htmlspecialchars(entry) + "</li>"
                            ...
                            for current_issue in plugin_obj["vulnerabilities"]:
                                issue_name = current_issue["title"]
                                issue_fix = "Upgrade plugin {} to version >= {}".format(
                                    plugin_name, current_issue["fixed_in"]
                                )
                                issue_cve = ",".join(current_issue["references"]["cve"])
                                issue_description = (
                                    "{}\n\nURLs:\n{}\n\nwpvulndb: {}".format(
                                        ...
                                        "\n".join(
                                            [
                                                " - " + x
                                                for x in current_issue["references"][
                                                    "url"
                                                ]
                                            ]
                                        ),
                                        ", ".join(
                                            current_issue["references"]["wpvulndb"]
                                        ),
                                    )
                                )
                                if "exploitdb" in current_issue:
...
def kubehunter_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/kubehunter.html",
        current_project=current_project,
            ...
            if file.filename:
                ...
                for node_obj in scan_result["nodes"]:
                    try:
                        ...
                        current_host = db.select_ip_from_project(
                            current_project["id"], node_ip
                        )
                        if current_host:
                            current_host = current_host[0]
                            db.update_host_description(current_host["id"], node_type)
                        else:
                            current_host = db.insert_host(
                                current_project["id"],
                                node_ip,
                                current_user["id"],
                                node_type,
                ...
                for service_obj in scan_result["services"]:
                    try:
                        ...
                        current_port = db.select_ip_port(
                            current_host["id"], service_port, is_tcp=True
                        )
                        if current_port:
                            current_port = current_port[0]
                            db.update_port_service(current_port["id"], service_info)
                        else:
                            current_port = db.insert_host_port(
                                current_host["id"],
                                service_port,
                                True,
                                service_info,
                ...
                for issue_obj in scan_result["vulnerabilities"]:
                    try:
                        ...
                        current_port = db.select_ip_port(
                            current_host["id"], issue_port, is_tcp=True
                        )
                        if current_port:
                            current_port = current_port[0]
                            db.update_port_service(
                                current_port["id"], form.ports_description.data
                            )
                        else:
                            current_port = db.insert_host_port(
                                current_host["id"],
                        ...
                        current_issue = db.insert_new_issue_no_dublicate(
                            issue_name,
                            issue_full_description,
                            "",
                            issue_cvss,
...
def burp_enterprise_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/burp_enterprise.html",
        current_project=current_project,
        ...
        if file.filename:
            ...
            for site_obj in site_array:
                ...
                if ip and not errors:
                    ...
                    current_port = db.select_ip_port(
                        current_host["id"], port, is_tcp=True
                    )
                    if current_port:
                        current_port = current_port[0]
                        db.update_port_service(current_port["id"], protocol)
                    else:
                        current_port = db.insert_host_port(
                            current_host["id"],
                            port,
                    ...
                    for issue_header_obj in rows_array:
                        i += 1
                        if (
                            "class" in issue_header_obj.attrs
                            and "issue-type-row" in issue_header_obj.attrs["class"]
                        ):
                            issue_name = issue_header_obj.find("td").string.split(" [")[
                                0
                            ]
                        else:
                            td_arr = issue_header_obj.find_all("td")
                            issue_path = issue_header_obj.find(
                                "td", {"class": "issue-path"}
                            ).string.strip()
                            dom_id = (
                                issue_header_obj.find("a")
                                .attrs["href"]
                                .replace("#", "")
                            )
                            ...
                            issue_container = scan_result.find(
                                "a", {"name": dom_id}
                            ).parent
                            issue_name = issue_container.find("h2").string
                            issue_description_container = issue_container.find("div")
                            issue_description_text = str(
                                issue_description_container.getText()
                            )
                            ...
                            # Remidiation == fix
                            issue_fix_short_header = issue_container.select(
                                'h3:contains("Remediation detail")'
                            )
                            issue_fix_short1_header = issue_container.select(
                                'h3:contains("Issue remediation")'
                            )
                            issue_fix = ""
                            ...
                            # References
                            issue_ref_header = issue_container.select(
                                'h3:contains("References")'
                            )
                            issue_ref = ""
                            if issue_ref_header:
                            ...
                            issue_class_header = issue_container.select(
                                'h3:contains("Vulnerability classifications")'
                            )
                            issue_class = ""
                            ...
                            # add issue
                            issue_full_description = (
                                issue_description_text + issue_ref + issue_class
                            )
                            ...
                            try:
                                ...
                                if current_hostname:
                                    services = {
                                        current_port["id"]: [current_hostname["id"]]
                                    }
                            except Exception as e:
                                pass
                            ...
                            # PoC Request
                            issue_request_header = issue_container.select(
                                'h3:contains("Request:")'
                            )
                            ...
                            # PoC Response
                            issue_response_header = issue_container.select(
                                'h3:contains("Response:")'
                            )
...
def dnsrecon_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/dnsrecon.html",
        current_project=current_project,
        tab_name="DNSrecon",
    )
    ...
    if not errors:
        ...
        for file in form.json_files.data:
            if file.filename:
                ...
                for hostname_row in scan_result:
                    hostname = (
                        hostname_row["target"] if "target" in hostname_row else ""
                    )
                    hostname_name = (
                        hostname_row["mname"] if "mname" in hostname_row else ""
                    )
                    host_ip = (
                        hostname_row["address"] if "address" in hostname_row else ""
                    )
                    host_port = hostname_row["port"] if "port" in hostname_row else ""
                    hostname_info = (
                        hostname_row["strings"] if "strings" in hostname_row else ""
                    )
                    hostname_type = (
                        hostname_row["type"] if "type" in hostname_row else ""
                    )
                    ...
                    if host_ip:
                        ...
                        try:
                            ...
                            for hostname_row_tmp in scan_result:
                                host_ip_tmp = (
                                    hostname_row["address"]
                                    if "address" in hostname_row
                                    else ""
                                )
                                domain_tmp = (
                                    hostname_row["mname"]
                                    if "mname" in hostname_row
                                    else ""
                                )
        ...
        for file in form.xml_files.data:
            if file.filename:
                ...
                for hostname_row in scan_result:
                    ...
                    hostname = (
                        hostname_row.get("target") if hostname_row.get("target") else ""
                    )
                    hostname_name = (
                        hostname_row.get("name") if hostname_row.get("name") else ""
                    )
                    host_ip = (
                        hostname_row.get("address")
                        if hostname_row.get("address")
                        else ""
                    )
                    host_port = (
                        hostname_row.get("port") if hostname_row.get("port") else ""
                    )
                    hostname_info = (
                        hostname_row.get("strings")
                        if hostname_row.get("strings")
                        else ""
                    )
                    hostname_type = (
                        hostname_row.get("type") if hostname_row.get("type") else ""
                    )
                    ...
                    if host_ip:
                        ...
                        try:
                            ...
                            for hostname_row_tmp in scan_result:
                                host_ip_tmp = (
                                    hostname_row_tmp.get("address")
                                    if hostname_row_tmp.get("address")
                                    else ""
                                )
                                domain_tmp = (
                                    hostname_row_tmp.get("name")
                                    if hostname_row_tmp.get("name")
                                    else ""
                                )
                                if host_ip_old == domain_tmp:
                                    try:
        ...
        for hostname in hostnames_dict:
            ...
            for ip_address in ip_array:
                ...
                if (":" not in ip_address) or (
                    ":" in ip_address and not form.ignore_ipv6.data
                ):

                    current_host = db.select_project_host_by_ip(
                        current_project["id"], ip_address
                    )
                    if not current_host:
                        host_id = db.insert_host(
                            current_project["id"],
                    ...
                    if not current_hostname:
                        hostname_id = db.insert_hostname(
                            host_id, hostname, description, current_user["id"]
                        )
                    else:
            ...
            if (":" not in ip_address) or (
                ...
                for port_num in ports_arr:
                    ...
                    if port_num_int > 0 and port_num_int < 65536:
                        current_port = db.select_host_port(
                            host_id, int(port_num), is_tcp=True
                        )
                        if not current_port:
                            port_id = db.insert_host_port(
                                host_id,
...
def theharvester_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/theharvester.html",
        current_project=current_project,
...
def metasploit_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/metasploit.html",
        current_project=current_project,
        tab_name="Metasploit",
    )
        ...
        for file in form.xml_files.data:
            if file.filename:
                ...
                for host_row in scan_result:
                    ...
                    host_os_detected_arch = host_row.find(
                        "detected-arch"
                    ).text  # x86_64
                    ...
                    if host_os_detected_arch and host_os_detected_arch != host_os_arch:
                        host_os_full += " Arch detected:{}".format(
                            host_os_detected_arch
                        )
                    ...
                    for port_row in services_arr:
                        ...
                        port_state = port_row.find(
                            "state"
                        ).text  # open closed filtered TODO: add option which port to add
                    ...
                    if form.add_nmap_scripts.data:
                        ...
                        for note_row in notes_arr:
                            script_name = note_row.find(
                                "ntype"
                            ).text  # nmap.nse.smb-os-discovery.host
                            if script_name not in [
                                "host.comments",
                                "host.info",
                                "host.os.nmap_fingerprint",
                                "host.name",
                            ]:
                                ...
                                try:
                                    script_data = (
                                        base64.b64decode(note_row.find("data").text)[
                                            16:
                                        ]
                                        .decode("charmap")
                                        .strip(" \n\t\r")
                ...
                for host_obj in hosts_dict:
                    current_host = db.select_project_host_by_ip(
                        current_project["id"], hosts_dict[host_obj]["ip"]
                    )
                    if current_host:
                        host_id = current_host[0]["id"]
                        if hosts_dict[host_obj]["description"]:
                            db.update_host_description(
                                host_id, hosts_dict[host_obj]["description"]
                            )
                        ...
                        host_id = db.insert_host(
                            current_project["id"],
                            hosts_dict[host_obj]["ip"],
                            current_user["id"],
                            hosts_dict[host_obj]["description"],
                            os=hosts_dict[host_obj]["os"],
                        )
                ...
                for port_obj in ports_dict:
                    current_port = db.select_host_port(
                        hosts_dict[ports_dict[port_obj]["host_report_id"]]["pcf_id"],
                    ...
                    if current_port:
                        ...
                        port_id = db.insert_host_port(
                            hosts_dict[ports_dict[port_obj]["host_report_id"]][
                                "pcf_id"
                            ],
                            ports_dict[port_obj]["port"],
                            ports_dict[port_obj]["is_tcp"],
                ...
                if not form.only_nmap.data:
                    ...
                    for website_obj in website_row:
                        ...
                        pcf_host_id = hosts_dict[
                            ports_dict[service_id]["host_report_id"]
                        ]["pcf_id"]
                        pcf_hostname_id = 0
                        if vhost:
                    ...
                    for vuln_obj in vuln_row:
                        ...
                        vuln_params = base64.b64decode(
                            vuln_obj.find("params").text
                        ).decode("charmap")[
                            4:
                        ]  # i dont know how to parse better
                        ...
                        services = {
                            web_dict[vuln_website_id]["pcf_port_id"]: [
                                web_dict[vuln_website_id]["pcf_hostname_id"]
                            ]
                        }
...
def nuclei_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/nuclei.html",
        current_project=current_project,
        tab_name="Nuclei",
    )
    ...
    for file in form.json_files.data:
        if file.filename:
            ...
            for issue_obj in json_data:
                ...
                issue_tags = (
                    "Tags: {}".format(", ".join(issue_obj["info"]["tags"]))
                    if issue_obj["info"]["tags"]
                    else ""
                )
                issue_description = (
                    issue_obj["info"]["description"]
                    if "description" in issue_obj["info"]
                    else ""
                )
                issue_references = (
                    "Links:\n{}".format(
                        "\n".join(
                            [" - {}".format(x) for x in issue_obj["info"]["reference"]]
                        )
                    )
                    if issue_obj["info"]["reference"]
                    else ""
                )
                issue_severity = "info"
                issue_matcher_name = (
                    "Matched: {}".format(issue_obj["matcher-name"])
                    if "matcher-name" in issue_obj
                    else ""
                )
                issue_cvss = 0.0
                if issue_severity == "low":
                    ...
                    issue_cvss = 10.0
                issue_type = (
                    "Script type: {}".format(issue_obj["type"])
                    if issue_obj["type"]
                    else ""
                )
                issue_curl_cmd = (
                    "Curl: {}".format(issue_obj["curl-command"])
                    if "curl-command" in issue_obj
                    else ""
                )
                issue_ip = (
                    issue_obj["ip"] if "ip" in issue_obj else ""
                )  # 142.250.185.78
                issue_host = (
                    issue_obj["host"] if "host" in issue_obj else ""
                )  # https://google.com
                issue_url = ""
                issue_protocol = (
                    issue_obj["protocol"] if "protocol" in issue_obj else ""
                )  # i dont know key "protocol
                issue_port = 0
                issue_hostname = ""
                ...
                for key_name in issue_obj:
                    if key_name not in blacklist_tags:
                        issue_other_fields += "{}: {}\n".format(
                            key_name, str(issue_obj[key_name])
                        )
...
def nmap_helper_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/helpers/nmap-helper.html",
        current_project=current_project,
...
def msfvenom_helper_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/helpers/msfvenom-helper.html",
        current_project=current_project,
...
def pingcastle_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/pingcastle.html",
        current_project=current_project,
        tab_name="PingCastle",
    )
...
def pingcastle_page_form(project_id, current_project, current_user):
    ...
    if not errors:
        ...
        for issue_obj in issues_database_xml.findAll("data"):
            issues_database[issue_obj.attrs["name"]] = issue_obj.findAll("value")[
                0
            ].text
        ...
        for file in form.xml_files.data:
            if file.filename:
                ...
                if domain_controllers:
                    for domain_obj in domain_controllers.findAll(
                        "healthcheckdomaincontroller"
                    ):
                        host_description = ""
                        host_os = (
                            ""
                            if not domain_obj.operatingsystem
                            else domain_obj.operatingsystem.text
                        )
                        if domain_obj.dcname:
                            host_description += "DC name: {}\n".format(
                                domain_obj.dcname.text
                            )
                        if domain_obj.lastcomputerlogondate:
                            host_description += "Last Logon: {}\n".format(
                                domain_obj.lastcomputerlogondate.text
                            )
                        if domain_obj.distinguishedname:
                            host_description += "Distinguished Name: {}\n".format(
                                domain_obj.distinguishedname.text
                            )
                        if domain_obj.ownersid:
                            host_description += "Owner SID: {}\n".format(
                                domain_obj.ownersid.text
                            )
                        if domain_obj.ownername:
                            host_description += "Owner Name: {}\n".format(
                                domain_obj.ownername.text
                            )
                        if (
                            domain_obj.hasnullsession
                            and domain_obj.hasnullsession == "true"
                        ):
                            host_description += "Has null session!\n"
                        if (
                            domain_obj.supportsmb1
                            and domain_obj.supportsmb1.text == "true"
                        ):
                            host_description += "Supports SMB1!\n"
                            if (
                                domain_obj.smb1securitymode
                                and domain_obj.smb1securitymode.text == "NotTested"
                            ):
                                host_description += "SMB1SecurityMode: {}\n".format(
                                    domain_obj.smb1securitymode.text
                                )
                        if (
                            domain_obj.supportsmb2orsmb3
                            and domain_obj.supportsmb2orsmb3.text == "true"
                        ):
                            host_description += "Supports SMBv2 or SMBv3.\n"
                        if domain_obj.smb2securitymode:
                            host_description += "SMB2 security mode: {}\n".format(
                                domain_obj.smb2securitymode.text
                            )
                        if (
                            domain_obj.remotespoolerdetected
                            and domain_obj.remotespoolerdetected.text == "true"
                        ):
                            host_description += "Detected remote spooler.\n"
                        if domain_obj.pwdlastset:
                            host_description += "Last pwd set: {}.\n".format(
                                domain_obj.pwdlastset.text
                            )
                        if domain_obj.rodc and domain_obj.rodc.text == "true":
                            host_description += "Read-Only DC\n"
                        if (
                            domain_obj.sysvoloverwrite
                            and domain_obj.sysvoloverwrite == "true"
                        ):
                            host_description += "SYSVOL overwrite\n"

                        if domain_obj.fsmo:
                            fsmo_result = ", ".join(
                                [x.text for x in domain_obj.fsmo.findAll("string")]
                            )
                            if fsmo_result:
                            ...
                            for host_ip_obj in ip_obj.findAll("string"):
                                ...
                                if current_host:
                                    ...
                                    current_host_id = db.insert_host(
                                        ...
                                        host_ip,
                                        current_user["id"],
                                        "Added from PingCastle",
                                        os=host_os,
                                    )
                                # add 88 port
                                current_port = db.select_host_port(
                                    current_host_id, port_num=88, is_tcp=True
                                )
                                if current_port:
                ...
                for risk_obj in risk_rules.findAll("healthcheckriskrule"):
                    ...
                    issue_riskid = risk_obj.riskid.text.replace(
                        "-", "_"
                    )  # A_AdminSDHolder
                    issue_briefly = risk_obj.rationale.text
                    issue_links = (
                        issues_database[issue_riskid + "_Documentation"].replace(
                            " ", ""
                        )
                        if (issue_riskid + "_Documentation") in issues_database
                        else ""
                    )
                    issue_purpose = (
                        issues_database[issue_riskid + "_Description"]
                        if (issue_riskid + "_Description") in issues_database
                        else ""
                    )
                    issue_fix = (
                        issues_database[issue_riskid + "_Solution"]
                        if (issue_riskid + "_Solution") in issues_database
                        else ""
                    )
                    issue_technical_description = (
                        issues_database[issue_riskid + "_TechnicalExplanation"]
                        if (issue_riskid + "_TechnicalExplanation") in issues_database
                        else ""
                    )
...
def maxpatrol_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/maxpatrol.html",
        current_project=current_project,
        tab_name="MaxPatrol",
    )
                ...
                for host in hosts_list.findAll("host"):
                    ...
                    for port_obj in scans.findAll("soft"):
                        ...
                        if 0 <= port <= 63353:
                            ...
                            current_port_id = db.select_host_port(
                                current_host_id, port, is_tcp
                            )
                            if current_port_id:
                                if port_service:
                                    db.update_port_proto_description(
                                        current_port_id[0]["id"],
                                        port_service,
                            ...
                            if port_issues:
                                for issue_obj in port_issues.findAll("vulner"):
                                    ...
                                    if issue_level > 0:
                                        ...
                                        issue_db_obj = vulns_db.find(
                                            "vulner", {"id": issue_db_id}
                                        )
                                        issue_name = issue_db_obj.title.text
                                        issue_short = (
                                            issue_db_obj.short_description.text
                                        )
                                        issue_description = (
                                            issue_db_obj.description.text
                                        )
                                        ...
                                        cvss3 = float(
                                            issue_db_obj.cvss3.attrs["base_score"]
                                        )
                                        cvss3_decomp = issue_db_obj.cvss3.attrs[
                                            "base_score_decomp"
                                        ].strip("()")

                                        if cvss3 == 0:
                                            cvss3 = float(
                                                issue_db_obj.cvss.attrs["base_score"]
                                            )

                                        issue_cve = ""
                                        issue_pub_date = (
                                            issue_db_obj.publication_date.text
                                            if issue_db_obj.publication_date
                                            else ""
                                        )
                                        if (
                                            issue_db_obj.global_id
                                            and "name" in issue_db_obj.global_id.attrs
                                            and issue_db_obj.global_id.attrs["name"]
                                            == "CVE"
                                        ):
                                            issue_cve = issue_db_obj.global_id.attrs[
                                                "value"
                                            ]

                                        # fstec fields
                                        issue_fstec = ""
                                        for fstec_obj in issue_db_obj.findAll(
                                            "global_id", {"name": "fstec"}
                                        ):
                                            issue_fstec += (
                                                fstec_obj.attrs["value"] + ","
                                            )
...
def scanvus_page(project_id, current_project, current_user):
    return render_template(
        "project/tools/import/scanvus.html",
        current_project=current_project,
        tab_name="Scanvus",
    )
...
def scanvus_page_form(project_id, current_project, current_user):
    ...
    if not errors:
        ...
        if form.host_id.data and is_valid_uuid(form.host_id.data):
            current_host = db.select_project_host(
                current_project["id"], form.host_id.data
            )
            if current_host:
                current_host = current_host[0]
            ...
            try:
                ...
                current_host = db.select_project_host_by_ip(
                    current_project["id"], form.ip.data
                )
                if current_host:
                    current_host = current_host[0]
                    host_id = current_host["id"]
                    ...
                    host_id = db.insert_host(
                        ...
                        form.ip.data,
                        current_user["id"],
                        comment=form.host_description.data,
                        os="Linux",
                    )
            except Exception as e:
                pass
