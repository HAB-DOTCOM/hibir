...
from app import check_session, check_team_access, db, send_log_data, config
...
def check_template_access(fn):
    ...
    def decorated_view(*args, **kwargs):
        ...
        current_template = db.check_user_issue_template_access(
            str(template_id), current_user["id"], current_user["email"]
        )
...
def check_rule_access(fn):
    ...
    def decorated_view(*args, **kwargs):
        ...
        current_user = kwargs["current_user"]
        current_rule = db.check_user_issue_rule_access(
            str(rule_id), current_user["id"], current_user["email"]
        )
        if not current_rule:
            return redirect("/projects/")
...
@routes.route("/manual.pdf", methods=["GET"])
@requires_authorization
@check_session
@check_password_reset
def documentation_manual(*args, **kwargs):
    return send_file(
        "configuration/manual.pdf", attachment_filename="Documentation.pdf"
    )
...
@requires_authorization
def login():
    if session.get("id") and session.get("pass_reset") == 0:
        return redirect("/projects/")
...
def login_form():
    ...
    if config["security"]["enable_form_login"] == "0":
        ...
        if not data:
            ...
            if not check_hash(data["password"], form.password.data):
                ...
                session.update(data)
        if not error:
            if db.select_user_by_email(form.email.data)[0]['pass_reset'] == 1:
    ...
    return render_template("login.html", form=form, error=error, tab_name="Login")


@routes.route("/password_reset", methods=["GET"])
@check_session
def PasswordResetPage(*args, **kwargs):
    return render_template("password_reset.html", tab_name="Password Reset")
    


@routes.route("/password_reset", methods=["POST"])
@check_session
def PasswordReset(current_user):
    ...
    if not errors:
        if not check_hash(current_user['password'], form.old_password.data):
            ...
            db.password_reset_protocol(form.new_password.data, current_user['id'])
            try:
                del session["id"]
                return redirect("/login")
            except Exception as e:
                errors.append("Unknown error has occurred. Please login again.")
    return render_template("password_reset.html", errors=errors, tab_name="Password Reset")


@routes.route("/register", methods=["GET"])
@requires_authorization
def register():
    return render_template("register.html", tab_name="Register")
...
def register_form():
    ...
    if config["security"]["enable_form_registration"] == "0":
        ...
        if len(db.select_user_by_email(form.email.data)) > 0:
            ...
            db.insert_user(form.fname.data, form.email.data, form.password1.data)
    else:
        error = "Wrong email format or empty password!"
    return render_template("register.html", form=form, error=error, tab_name="Register")
...
def profile(current_user):
    return render_template("profile.html", user_data=current_user, tab_name="Profile")
    ...
    # editing profile password
        ...
        if form2.errors:
            ...
            if form2.oldpassword.data == form2.password1.data:
                ...
                if not check_hash(current_user["password"], form2.oldpassword.data):
                    errors.append("Wrong password!")
                    error_type = "change_password"
                else:
                    db.update_user_password(current_user["id"], form2.password1.data)
        ...
        if add_config_form.errors:
            ...
            if add_config_form.action.data == "Add":
                ...
                # check if exist
                same_config = db.select_configs(
                    user_id=current_user["id"], name=config_name
                )
                if same_config:
                    db.update_config(
                ...
                db.delete_config(
                    user_id=current_user["id"], name=add_config_form.config_name.data
                )
        ...
        if delete_report_form.errors:
            ...
            current_template = db.select_report_templates(
...
def create_team(current_user):
    return render_template("new_team.html", tab_name="Create team")
...
def team_logs(team_id, current_team, current_user):
    return (
        render_template(
            "team_logs.html",
            current_team=current_team,
            tab_name=current_team["name"] + " logs"
    ...
    # team tester add
        ...
        if team_user_add_form.errors:
            ...
            if not user_to_add:
                ...
                errors = [
                    "User already added to {} group!".format(
                        current_team_users[user_to_add[0]["id"]]
                    )
                ]
            else:
                db.update_new_team_user(
                    current_team["id"],
        ...
        if add_config_form.errors:
            ...
            if add_config_form.action.data == "Add":
                ...
                # check if exist
                same_config = db.select_configs(
                    team_id=current_team["id"], name=config_name
                )
                if same_config:
                    db.update_config(
        ...
        if delete_report_form.errors:
            ...
            template_id = delete_report_form.template_id.data
            current_template = db.select_report_templates(
                template_id=template_id, team_id=current_team["id"]
            )
            if not current_template:
                errors.append("Template not found!")
            else:
                current_template = current_template[0]
                db.delete_template_safe(
                    template_id=current_template["id"], team_id=current_team["id"]
                )
        return render_template(
            "team.html",
            current_team=current_team,
...
def new_project_form(current_user):
    ...
    if form.errors:
        for field in form.errors:
            for err in form.errors[field]:
                errors.append(err)
        return render_template(
            "new_project.html", errors=errors, tab_name="New project"
        )
    ...
    project_id = db.insert_new_project(
        form.name.data,
        form.description.data,
        form.project_type.data,
        form.scope.data,
        form.out_of_scope.data,
        start_time,
        end_time,
        form.archive.data,
        form_users,
...
@routes.route("/team/<uuid:team_id>/user/<uuid:user_id>/<action>", methods=["POST"])
@requires_authorization
@check_session
@check_password_reset
@check_team_access
...
def list_projects(current_user):
    return render_template("projects.html", tab_name="Projects")
...
def new_issue_templates(current_user):
    return render_template("new_issue_template.html", tab_name="New template")
...
def new_issue_templates_form(current_user):
    ...
    if not errors:
        ...
        for field_name in form.additional_field_name.data:
            ...
            if field_type in ["text", "number", "float", "boolean"]:
                ...
                try:
                    ...
                    add_fields_dict[field_name]["value"] = (
                        type_func(field_value)
                        if field_type == "text" or field_value
                        else type_func(0)
                    )
        ...
        template_id = db.insert_new_issue_template(
            form.template_name.data,
            form.name.data,
            form.description.data,
            form.url.data,
            cvss,
    ...
    return render_template(
        "new_issue_template.html", tab_name="New template", errors=errors
    )
...
def new_issue_rule(current_user):
    return render_template(
        "new_issue_rule.html", tab_name="New rule", current_user=current_user
    )
...
def new_issue_rule_form(current_user):
    ...
    for search_rule in rule_json_search:
        search_obj = {
            ...
            "type": search_rule["rule_type"]
            if search_rule["rule_type"] == "regexp"
            else "substring",
    ...
    for replace_rule in rule_json_replace:
        ...
        if "id" in replace_rule and is_valid_uuid(replace_rule["id"]):
            current_template = db.check_user_issue_template_access(
                replace_rule["id"], current_user["id"], current_user["email"]
            )
            if current_template:
                template_obj = {
                    "type": "template",
...
def import_issue_templates(current_user):
    return render_template("import_issue_templates.html", tab_name="Import template")
...
def import_issue_templates_form(current_user):
            ...
            if file.filename:
                try:
                    ...
                    for template_obj in templates_arr:
                        ...
                        template_cvss = (
                            float(template_obj["cvss"])
                            if float(template_obj["cvss"]) >= 0
                            and float(template_obj["cvss"]) <= 10
                            else 0
                        )
                        template_cwe = (
                            int(template_obj["cwe"])
                            if int(template_obj["cwe"]) >= 0
                            else 0
                        )
                        ...
                        for field_name in template_fields:
                            # first check
                            if (
                                type(template_fields[field_name]) == dict
                                and "type" in template_fields[field_name]
                                and "value" in template_fields[field_name]
                            ):
                                # second check
                                if template_fields[field_name]["type"] in [
                                    "text",
                                    "number",
                                    "float",
                                    "boolean",
                                    ...
                                    if template_fields[field_name]["type"] == "text":
                                        ...
                                        template_fields[field_name]["type"] == "number"
                                    ):
                                        type_func = int
                                    elif template_fields[field_name]["type"] == "float":
                                        type_func = float
                                    elif (
                                        template_fields[field_name]["type"] == "boolean"
                                    ):
                                        type_func = lambda x: bool(int(x))
                        ...
                        for variable_name in template_variables:
                            # first check
                            if (
                                type(template_variables[variable_name]) == dict
                                and "type" in template_variables[variable_name]
                                and "value" in template_variables[variable_name]
                            ):
                                # second check
                                if template_variables[variable_name]["type"] in [
                                    "text",
                                    "number",
                                    "float",
                                    "boolean",
                                    ...
                                    if (
                                        template_variables[variable_name]["type"]
                                        == "text"
                                    ):
                                        type_func = str
                                    elif (
                                    ...
                                    template_variables_checked[str(variable_name)] = {
                                        "type": template_variables[variable_name][
                                            "type"
                                        ],
                                        "value": type_func(
                                            template_variables[variable_name]["value"]
                                        ),
                                    }
    ...
    return render_template(
        "import_issue_templates.html", tab_name="Import template", errors=errors
    )
    ...
    if not errors:
        ...
        if not current_issue:
            ...
            current_project = db.check_user_project_access(
                current_issue["project_id"], current_user["id"]
            )
            if not current_project:
                errors.append("You do not have access to this project!")
            else:
                return render_template(
                    "new_issue_template_from_issue.html",
                    tab_name="Edit template",
...
def download_issue_rules(current_user):
    ...
    for rule_id in form.rule_id.data:
        current_rule = db.check_user_issue_rule_access(
            rule_id, current_user["id"], current_user["email"]
        )
        if current_rule:
...
def import_issue_rules(current_user):
    return render_template("import_issue_rules.html", tab_name="Import rules")
...
def import_issue_rules_form(current_user):
                    ...
                    for rule_obj in rules_arr:
                        ...
                        for search_rule in rule_obj["search_rules"]:
                            new_obj = {
                                ...
                                "type": str(search_rule["type"])
                                if str(search_rule["type"]) == "regexp"
                                else "substring",
                        ...
                        for extract_rule in rule_obj["extract_vars"]:
                            new_obj = {
                                ...
                                "type": str(extract_rule["type"])
                                if str(extract_rule["type"]) == "regexp"
                                else "substring",
                        ...
                        for replace_rule in rule_obj["replace_rules"]:
                            ...
                            if replace_rule["type"] != "template":
                                new_obj = {
                                    "type": str(replace_rule["type"])
                                    if str(replace_rule["type"]) == "regexp"
                                    else "substring",
                                    "search_filter": str(replace_rule["search_filter"]),
                                    "replace_string": str(
                                        replace_rule["replace_string"]
                                    ),
                                ...
                                if is_valid_uuid(
                                    template_id
                                ) and db.check_user_issue_template_access(
                                    template_id,
                                    current_user["id"],
                                    current_user["email"],
                                ):
                                    new_obj = {
                                        ...
                                        "vars": {
                                            str(var_name): str(
                                                replace_rule["vars"][var_name]
                                            )
                                            for var_name in replace_rule["vars"]
                                        },
                                        "type": "template",
                                    }
                                else:
                                    ...
                                    all_available_templates = (
                                        db.select_all_user_issue_templates(
                                            current_user["id"],
                                            current_user["email"],
                                            team_uuid,
                                        )
                                    ...
                                    replace_rule["md5"] = (
                                        str(replace_rule["md5"])
                                        if "md5" in replace_rule
                                        else ""
                                    )
                                    # calculate md5
                                    found_template_id = ""
                                    for current_template in all_available_templates:
                                        md5_str = md5_hex_str(
                                            ...
                                            + "\n"
                                            + "url_path: "
                                            + current_template["url_path"]
                                            + "\n"
                                            + "cvss: "
                                            + str(current_template["cvss"])
                                            + "\n"
                                            + "cwe: "
                                            + str(current_template["cwe"])
                                            + "\n"
                                            + "cve: "
                                            + current_template["cve"]
                                            + "\n"
                                            + "status: "
                                            ...
                                            + "\n"
                                            + "fix: "
                                            + current_template["fix"]
                                            + "\n"
                                            + "param: "
                                            + current_template["param"]
                                            + "\n"
                                        )
                            ...
                            if new_obj:
                                replace_rules_json.append(new_obj)
    ...
    return render_template(
        "import_issue_rules.html", tab_name="Import rule", errors=errors
    )
...
def delete_issue_rule_form(current_user, rule_id, current_rule):
    db.delete_issue_rule(current_rule["id"])
    redirect = (
        "/team/{}/#/configs".format(current_rule["team_id"])
        if current_rule["team_id"]
        else "/profile#/config"
    )
