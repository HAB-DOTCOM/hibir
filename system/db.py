...
class Database:
    ...
    def return_arr_dictny(self, query, params=None):
    ...
    def insert_user(self, fname, email, password):
        ...
        self.execute(
            "INSERT INTO Users(id,fname,email,password,date_added) VALUES (?,?,?,?,?)",
            (user_id, fname, email, password_hash, current_date)
        )
        self.conn.commit()
        return user_id
    ...
    def password_reset_protocol(self, password, user_id):
        password_hash = hash_password(password)
        self.execute('UPDATE Users SET password=?,pass_reset=0 WHERE id=?', (password_hash, user_id))
        self.conn.commit()

    def reset_password(self, password, user_id):
        password_hash = hash_password(password)
        self.execute('UPDATE Users SET password=?,pass_reset=1 WHERE id=?', (password_hash, user_id))
        self.conn.commit()
    ...
    def update_score(self, project_rating, project_level, user_id):
        self.execute(
        ''' UPDATE Users SET score=? WHERE id? 
        ''', (project_rating * project_level, user_id))
        self.conn.commit()
    ...
    def insert_log(self, description, user_id='', teams=[], project_id='',
        ...
        self.execute(
            '''INSERT INTO Logs(id, teams, description, date, user_id, project_id, date_added) 
            VALUES (?, ?, ?, ?, ?, ?, ?)''',
            (gen_uuid(), json.dumps(teams), description, int(time.time()), user_id, project_id, date_added)
        )
        self.conn.commit()
        return
    
    def get_all_teams(self):
        self.execute("SELECT * FROM Teams")
        result = self.return_arr_dict()
        return result
    ...
    def insert_new_project(self, name, description, project_type, scope, out_of_scope,
                       start_date, end_date, auto_archive, testers,
                       teams, admin_id, user_id, folder, classification, level, report_title):
        project_id = gen_uuid()
        date_added = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        self.execute(
            "INSERT INTO Projects(id,name,description,project_type,scope, out_of_scope, start_date,"
            "end_date,auto_archive,status,testers,teams,admin_id, folder, classification, level, report_title, date_added) "
            "VALUES (?,?,?,?,?,?,?,?,?,1,?,?,?,?,?,?,?,?)",
            (project_id,
            name,
            description,
            ...
            level,
            report_title,
            date_added)  # initiation of json dict
        )
        self.conn.commit()
    ...
    def get_user_score(self,user_id):
        ...
        for project in projects:
            if project['status'] == 0:
                score +=project['rating']*project['level']
        return score
    ...
    def get_project(self, project_id):
        ...
        result = self.return_arr_dict()
        return result
    def get_project_testers(self, project_id):
        self.execute(
        '''SELECT testers FROM Projects WHERE id=?''',
        (str(project_id),)
        )
        result = self.return_one_dict()
        if result:
            testers = result.get("testers")
            if testers:
                return [{"user_id": tester} for tester in testers.split(",")]
        return []
    def get_user(self,user_id):
        ...
        result = self.return_one_dict()
        return result
    def update_user_score(self, user_id, total_score):
        self.execute(
    ...
    def select_all_archived_projects(self):
        self.execute(
            '''SELECT * FROM Projects where status=0 '''
        )
        result = self.return_arr_dict()
        return result
    ...
    def check_user_project_access(self, project_id, user_id):
        ...
        if user and user['admin'] == 1:
            proj =self.select_projects(project_id)
        ...
        for user_project in user_projects:
            if user_project['id'] == project_id:
                return user_project
        return None
    def return_dict(self, query, params=None):
    ...
    def get_project_counters(self, project_id):
        project_id_str = str(project_id)
        result = self.return_arr_dictny("SELECT COUNT(*) as hosts_counter FROM Hosts WHERE project_id=?", (project_id_str,))
        return result[0] if result else {"hosts_counter": 0}
    ...
    def insert_host(self, project_id, ip, user_id,
        ...
        ip_id = gen_uuid()
        self.execute(
            '''INSERT INTO Hosts(id,project_id,ip,comment,user_id,threats,os,date_added) 
    ...
    def insert_host_multiple(self, project_id, ip_list, user_id, comment_list, threats_list, os_list):
        ...
        self.executemany('''INSERT INTO Hosts(id,project_id,ip,user_id,comment,threats,os,date_added) 
            VALUES (?,?,?,?,?,?,?,?)''', insert_list)
        self.conn.commit()
    ...
    def insert_host_port_multiple(self, user_id, project_id, host_id_list=[], port_list=[],
        ...
        self.executemany(
            '''INSERT INTO Ports(
            id,host_id,port,is_tcp,service,description,user_id,project_id,date_added) 
            VALUES (?,?,?,?,?,?,?,?,?)''', insert_list
        )
        self.conn.commit()
    ...
    def insert_hostnames_multiple(self, host_ids_list, hostnames_list, descriptions_list, user_id):
        ...
        self.executemany(
            ...
            id,host_id,hostname,description,user_id,date_added) 
               VALUES (?,?,?,?,?,?)''',
            insert_list
        )
        self.conn.commit()
    ...
    def insert_issues_multiple(self, name_list, description_list, url_path_list, cvss_list, user_id,
        ...
        self.executemany(
            '''INSERT INTO Issues(
            id,name,description,url_path,cvss,user_id,services,status, project_id, cve,cwe,type,fix,param,fields, date_added) 
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
            insert_list
        )
        self.conn.commit()
    ...
    def update_port_service_multiple(self, port_ids_list, service_list):
        ...
        if not port_ids_list:
            return
        for x in range(len(port_ids_list)):
            sql += ' WHEN id=? THEN ? '
            update_list += [port_ids_list[x], service_list[x]]
        sql += ' WHEN id IN ({}) THEN service END WHERE id in ({}) '.format(','.join(['?' for x in port_ids_list]), ','.join(['?' for x in port_ids_list]))
        update_list += port_ids_list
        update_list += port_ids_list
        self.execute(sql, update_list)
        self.conn.commit()
    ...
    def insert_host_port(self, host_id, port, is_tcp, service, description, user_id, project_id):
        # check port in host
        ...
        self.execute(
            '''INSERT INTO Ports(
            id,host_id,port,is_tcp,service,description,date_added,user_id,project_id) 
               VALUES (?,?,?,?,?,?,?,?,?)''',
            (port_id, host_id, port, int(is_tcp), service, description, date_added, user_id, project_id)
        )
        self.conn.commit()
        if port != 0:
    ...
    def insert_new_issue(self, name, description, url_path, cvss, user_id,
        ...
        self.execute(
            '''INSERT INTO Issues(
            id, name, description, url_path, cvss, user_id, services, status,
            project_id, cve, cwe, type, fix, param, fields, date_added) 
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
            (issue_id, name, description, url_path, cvss, user_id,
            json.dumps(services), status, project_id, cve, cwe, issue_type,
            fix, param, json.dumps(fields), date_added)
        )
        self.conn.commit()
    ...
    def update_issue_with_fields(self, issue_id, name, description, url_path, cvss,
        ...
        self.execute(
            '''UPDATE Issues SET name=?, description=?, url_path=?, cvss=?, 
            services=?, status=?, cve=?, cwe=?, fix=?, type=?, param=?, fields=? WHERE id=?''',
            (name, description, url_path, cvss,
             json.dumps(services), status, cve, cwe, fix, issue_type,
             param, json.dumps(fields), issue_id)
        )
        self.conn.commit()
        self.insert_log('Updated issue {}'.format(name))
    ...
    def insert_new_poc(self, port_id, description, file_type, filename, issue_id, user_id, hostname_id, poc_id='random', storage='filesystem', data=b''):
        if poc_id == 'random':
            poc_id = gen_uuid()
        ...
        self.execute(
            ...
            issue_id,user_id,hostname_id,priority,storage, base64, date_added) 
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?)''',
            (poc_id, port_id, description, file_type, filename, issue_id,
            user_id, hostname_id, priority, storage, base64.b64encode(data).decode('charmap'), date_added)
        )
        self.conn.commit()
        current_issue = self.select_issue(issue_id)[0]
        self.insert_log('Added PoC {} to issue {}'.format(poc_id, current_issue['name']))
        return str(poc_id)
    ...
    def insert_new_network(self, ip, mask, asn, comment, project_id, user_id,
        ...
        self.execute(
            ...
            id,ip,mask,comment,project_id,user_id,is_ipv6,asn,internal_ip, cmd, access_from, name, date_added) 
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)''',
            (network_id, ip, mask, comment, project_id, user_id, int(is_ipv6),
            int(asn or 0), internal_ip, cmd, json.dumps(services), name, date_added)
        )
    ...
    def insert_new_cred(self, login, password_hash, hash_type,
        ...
        self.execute(
            ...
            description,source,services, user_id, project_id, date_added)
             VALUES (?,?,?,?,?,?,?,?,?,?,?)''',
            (cred_id, login, password_hash, hash_type, cleartext_passwd,
             description, source, json.dumps(services), user_id, project_id, date_added)
        )
        self.conn.commit()
    ...
    def insert_new_note(self, project_id, name, user_id, host_id='', text=''):
        ...
        self.execute(
            '''INSERT INTO Notes(
            id, project_id, name, text, user_id, host_id, date_added) 
            VALUES (?, ?, ?, ?, ?, ?, ?)''',
            (note_id, project_id, name, text, user_id, host_id, date_added)
        )
        self.conn.commit()
    ...
    def insert_new_file(self, file_id, project_id, filename, description, services, filetype, user_id, storage, data=b''):
        date_added = datetime.now().strftime('%Y-%m-%d %H:%M:%S') # get the current date and time
        self.execute(
            '''INSERT INTO Files(
            id, project_id, filename, description,
            services, type, user_id, storage, base64, date_added) 
            VALUES (?,?,?,?,?,?,?,?,?,?)''',
    ...
    def update_project_settings(self, project_id, name, description,
                                project_type, scope, out_of_scope,
                                start_date, end_date, auto_archive, testers,
                                teams, classification, level, admin_id, folder, report_title, rating):
        self.execute(
            "UPDATE Projects SET name=?,description=?,project_type=?,"
            "scope=?,out_of_scope=?,start_date=?,"
            "end_date=?,admin_id=?,auto_archive=?,"
            "testers=?,teams=?,folder=?,classification=?,level=?,report_title=?,rating=? WHERE id=?",
            (name,
             description,
             project_type,
             scope,
    ...
    def update_project_rating(self, project_id, rating):
        self.execute(
            '''UPDATE Projects SET rating=? WHERE id=? ''', (rating, project_id)
        )
        self.conn.commit()
        self.insert_log("Project got a rating of {}".format(int(rating)))
    ...
    def insert_chat(self, project_id, name, user_id):
        ...
        self.execute(
            '''INSERT INTO Chats(
            id,project_id,name,user_id,date_added) 
            VALUES (?,?,?,?,?)''',
            (chat_id, project_id, name, user_id, date_added)
        )
        self.conn.commit()
    ...
    def insert_new_message(self, chat_id, message, user_id):
        ...
        self.execute(
            ...
            id,chat_id,message,user_id,time, date_added) 
            VALUES (?,?,?,?,?,?)''',
            (message_id, chat_id, message, user_id, message_time, date_added)
        )
        self.conn.commit()
        current_chat = self.select_chat(chat_id)
    ...
    def insert_new_http_sniffer(self, name, project_id):
        ...
        self.execute(
            ...
            id,project_id,name,status,location,body,date_added) 
            VALUES (?,?,?,?,?,?,?)''',
            (sniffer_id, project_id, name, 200, '', '', date_added)
        )
        self.conn.commit()
    ...
    def insert_new_http_sniffer_package(self, sniffer_id, date, ip, request):
        ...
        self.execute(
            ...
            id,sniffer_id,date,ip,request,date_added) VALUES (?,?,?,?,?,?)''',
            (package_id, sniffer_id, date, ip, request, date_added)
        )
        self.conn.commit()
    ...
    def insert_config(self, team_id='0', user_id='0', name='', display_name='',
        ...
        self.execute(
            '''INSERT INTO Configs(
            id,date_added,team_id,user_id,name,display_name,
            data,visible) VALUES (?,?,?,?,?,?,?,?)''',
            (config_id, date_added, team_id, user_id, name, display_name, data, visible)
        )
        self.conn.commit()
    ...
    def insert_template(self, template_id='', team_id='0', user_id='0', name='',
        ...
        self.execute(
            ...
            id,team_id,user_id,name,filename, storage, base64, date_added) 
            VALUES (?,?,?,?,?,?,?,?)''',
            (template_id, team_id, user_id, name, filename, storage, 
            base64.b64encode(data).decode('charmap'), date_added)
        )
        self.conn.commit()
    ...
    def select_report_info_sorted(self, project_id):
        ...
        tmp_result['project_type'] = current_project['project_type']
        tmp_result['scope'] = current_project['scope']
        tmp_result['out_of_scope'] = current_project['out_of_scope']
        tmp_result['classification'] = current_project['classification']
    ...
    def insert_token(self, user_id, name='', create_date=0, duration=60 * 60 * 24):
        token_id = gen_uuid()
        date_added = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        self.execute(
            '''INSERT INTO Tokens(
            id,user_id,name,create_date,duration,date_added) 
            VALUES (?,?,?,?,?,?)''',
            (token_id, user_id, name, create_date, duration, date_added)
        )
        self.conn.commit()
    ...
    # def get_all_issues(self, project_id, page=1, per_page=10):
    ...
    #     paginated_issues = Pagination(page=page, per_page=per_page, total=total_count, items=items, query='/project/{}/issues/'.format(project_id))

    #     return total_count, items, paginated_issues
    ...
    # def get_issues_by_project_id_pag(self, project_id, page, per_page):
    #     query = 'SELECT COUNT(*) FROM Issues WHERE project_id = ?'
    #     self.execute(query, (str(project_id),))
    #     result = self.return_arr_dict()
    #     total_count = result[0]['COUNT(*)']

    #     offset = (page - 1) * per_page
    #     query = 'SELECT * FROM Issues WHERE project_id = ? LIMIT ? OFFSET ?'
    #     self.execute(query, (str(project_id), per_page, offset))
    #     result = self.return_arr_dict()
    ...
    def insert_new_issue_template(self, tpl_name, name, description, url_path, cvss, status, cve='', cwe=0,
        ...
        self.execute(
            ...
            id, tpl_name, name, description, url_path, cvss, cwe, cve, status, type, fix, param, fields, variables, user_id, team_id, date_added) VALUES
            (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',
            (str(template_id), tpl_name, name, description, url_path,
            cvss, cwe, cve, status, issue_type, fix, param,
            json.dumps(fields), json.dumps(variables), user_id, team_id, date_added)
        )
        self.conn.commit()
    ...
    def insert_path(self, project_id='', out_host='', out_network='', in_host='', in_network='', description='', path_type='connection', direction='forward'):
        ...
        self.execute(
            ...
            id,host_out,network_out,host_in,network_in,description,project_id,type, direction, date_added) 
            VALUES (?,?,?,?,?,?,?,?,?,?)''',
            (path_id, out_host, out_network, in_host, in_network, description, project_id, path_type, direction, date_added)
        )
        self.conn.commit()
    ...
    def insert_issue_rule(self, name, team_id, user_id, search_rules_arr, extract_vars_arr, replace_rules_arr):
        ...
        self.execute(
            ...
            id,name, team_id, user_id, search_rules, extract_vars, replace_rules, date_added) 
            VALUES (?,?,?,?,?,?,?,?)''',
            (rule_id, name, team_id, user_id, json.dumps(search_rules_arr),
            json.dumps(extract_vars_arr), json.dumps(replace_rules_arr), date_added)
        )
        self.conn.commit()
    ...
    def select_global(self, search_field, user_id, case_sensetive=False):
        ...
        self.execute(
            ...
            (
                ...
                search_field, search_field, search_field, search_field, search_field,
                search_field, search_field, search_field, search_field, search_field,
                search_field, search_field, search_field, search_field, search_field, search_field, search_field, search_field, search_field, search_field,
                ...
                search_field, search_field, search_field,
                user_id, search_field, search_field, search_field, search_field, search_field, search_field, search_field, search_field, search_field, search_field, search_field,
